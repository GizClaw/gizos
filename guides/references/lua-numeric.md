...
Prepared constraints, weighted rotations and geometry poses use f64 state and descriptors; only the explicit displacement export and float-mode coefficient inputs below use f32. They preserve binary64 world coordinates and selectively use the extracted float/compensated arithmetic; they do not change any existing f32/f64 operation. Their complete signatures, capacities, precision, reset and failure rules are specified in the generated [numeric API reference](./lua.md). Direct pose/polyline drawing and RGB888 gradient descriptors are specified in the [Display API reference](./lua.md).
...
`displacements` writes a selected node range to a reusable f32 xyz prefix by subtracting double current/previous values before casting. Displacement-f32 integration accepts before/gain0/gain1 independently as f32 or f64, while mobility, after and bounds remain f64. The separate float products and double state updates retain their order. Lua owns drag/environment formulas and keeps world-coordinate threshold decisions in f64; choosing f32 coefficient channels does not authorize rounding those decisions. The f64 integration mode remains all-f64.

`length3_refined` is an explicit f64 bulk norm using the extracted float square-root seed plus one double correction. Its refined squared domain is [1e-20,1e20], with relative error at most 1e-12 versus double square root; outside it the original double libm fallback applies, including possible squared underflow to zero. It stages results and supports source/output aliases. Default `length3` and `normalize3` retain their existing behavior, including tiny-vector scaling. Callers explicitly select the arithmetic path; this entrypoint does not normalize or select application vectors.
...
The prepared translation units, `h2_lua_vmath.c`, `h2_lua_geometry.c` and the isolated `h2_lua_display.c` use `-O3 -fno-fast-math` on GNU-compatible toolchains, preserving explicit `fmaf`; the source-package dependency aspect exports the same flags per compilation unit. Lua core, Runtime, the remaining modules and the separate raster library retain their existing compile flags. The independent package test checks this per-unit boundary and those flags and compiles/loads the real built-ins. Existing Host numeric/raster harnesses cover complete state and pixel comparisons, failure atomicity, bounded storage, warm allocation, ownership and teardown; the existing browser fixture exercises the same standard Host. Numeric timing records setup, first execution, same-input native reference and warm phase bindings separately. Host timing is not a target FPS guarantee; paired downstream device evidence is still required before closing #383.

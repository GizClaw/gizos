...
Reliable UART transport 默认使用 `460800` baud，Loader 与 APP image 使用相同 contract；固件启动时直接应用编译期 ESP sdkconfig 或 BK AP/CP defaults，Host 未传 `--baud` 时自动使用同一默认值。全局 `--baud RATE` 只用于连接另行配置的镜像。macOS Host 对 termios 没有具名常量的非零 rate 使用 `IOSSIOSPEED` 配置，底层 endpoint 不接受 custom rate 时返回 `UNSUPPORTED`。Native USB Serial/JTAG 不使用 UART baud；为该 endpoint 传入不同 `--baud` 值不会改变 USB 链路速率。Host Serial 打开 endpoint 后、借出 byte stream 前明确 deassert DTR/RTS；endpoint 返回 canonical `UNSUPPORTED` 时继续，任何其它 control-line 错误立即终止连接。每次 disconnect 都发送 `SESSION_CLOSE`，设备释放旧 session 后下一次 open 使用新的非零 session ID 握手。Host、ESP UART backend 和 BK AP/CP tunnel 都保留一次 encoded frame 的写边界，不增加固定 write gap；设备 console 与 protocol frame 通过 target TX serializer 串行化。进度与最终成功仍以 peer ACK 和 package checksum 为准。BootROM recovery 和板载 modem UART 使用各自独立的速率，不受该 managed command transport 默认值影响。

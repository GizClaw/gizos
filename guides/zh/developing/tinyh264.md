...
Allocator 选择使用调用方持有的栈上 scope；默认通过 `_Thread_local` 隔离并发 decoder 线程的 allocator 指针，嵌套 scope 退出时恢复前一个 allocator。没有可用 TLS 的 target port 可定义 `H2_TINYH264_SCOPE_TASK`（返回当前 task identity）和 `H2_TINYH264_SCOPE_YIELD`（按毫秒 sleep/yield），切换到按 task identity 索引的 scope node registry；短时 atomic gate 仅在链接、移除和查找 node 时持有。

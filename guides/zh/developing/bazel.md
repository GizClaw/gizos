...
调用方由 `.env/devenv` 提供 `BK7258_PATH`。`--config=bk7258` 仅用 `--repo_env` 将该 locator source 交给 `@h2_bk7258_sdk`；repository rule 验证 exact SDK commit `d7187c8fc75c56930b0106ec55ff770c2d5a4a93` 与 tracked cleanliness。`@h2_bk_arm_toolchain` 根据 host 从 Arm 官方地址下载 GNU 10.3-2021.10，先校验 archive SHA-256，再校验完整解压树摘要；macOS arm64 使用 x86_64 archive，并在 repository evaluation 时 fail closed 验证 Rosetta。Native action 只接收两个 locator 和 Bazel Python runtime，不继承 `BK_TOOLCHAIN_ARCHIVE`、`COMPILER_TOOLCHAIN_PATH`、caller `PATH` 或完整 environment。Runner 发布 outputs 前重新验证 SDK identity 与 cleanliness，并确认 GizOS checkout 未变化。

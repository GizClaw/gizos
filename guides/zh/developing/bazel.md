...
`jieli_firmware` 用一个 rule 覆盖杰理各系列：`target = br23`（AC695N，Lucky Kitty 的 AC6956C）与 `target = wl82`（AC791N）。Firmware entry 只为仓库自己的 portable App 建立，位于 `projects/<project>/targets/jieli_firmware/<image>/<board>/`。`--config=ac695n` / `--config=ac791n` 仅用 `--repo_env` 把 `JIELI_AC695N_SDK_PATH` / `JIELI_AC791N_SDK_PATH` 交给对应 `@h2_jieli_<family>_sdk`，并把 `JIELI_TOOLCHAIN_ROOT`、`JIELI_POSTBUILD_ROOT` 交给共用的 `@h2_jieli_toolchain` 与 `@h2_jieli_postbuild`；这两个 repository 不下载（上游短链不可 pin），而是用 `toolchain_identity.py` 按 `tools/bazel/native_versions/jieli_toolchain_archives.txt` 校验 firmware-devenv 解包出的 expanded tree。SDK repository 验证 `native_versions/jieli_<family>_sdk_commit.txt` exact commit 与 tracked cleanliness。

Runner 把 pinned SDK 子树复制到 invocation-local 目录，用 Bazel 验证过的 `pi32v2/bin` 覆盖 SDK Makefile 的 `TOOL_DIR`，通过 `tools/bazel/jieli/h2_sdk_wrapper.mk` 的 `h2_link` target 驱动 SDK Makefile：SDK 对象照常由 SDK 规则编译，`firmware_native_component` 源文件用 SDK flags 编译，`firmware_lib_component` 的 archive 进入同一条链接（跳过官方走 host-client 云打包的 `download.sh`），再执行仓库自有 `tools/bazel/jieli/local_post_<target>.sh` 用 Linux `isd_download`/`fw_add`/`ufw_maker` 本地出包。Portable archive 由 `@h2_jieli_pi32v2_cc_toolchain`（`local_embedded_cc` 的 clang 形态：`compiler_kind = "clang"`、`tool_overrides`、`system_include_dirs`、`exec_hosts = ["linux_x86_64"]`）编译为非 LTO ELF 对象，`H2_WARNING_COPTS` 在 `is_pi32v2` 下关闭 clang 4.0.1 对 `{0}` 的误报。每个 target 固定返回 `firmware/firmware.elf`、`symbols.txt`、`jl_isd.bin`、`jl_isd.fw`、`update.ufw` 与 `manifest.json`；`JieliFirmwareInfo`、`DefaultInfo.files` 与 `OutputGroupInfo.release` 暴露相同文件，不返回 `FirmwareReleaseInfo`。Action 在当前 runner 上 unsandboxed、non-remote-exec 执行但不设置 `local`，声明 4 CPU / 4 GiB，commit 文件、archive identity 文件、locator 与 post 脚本都是显式 action input，成功结果进入 local/GCS cache，不执行设备操作。
...
macOS CI 和 Release 明确选择 Xcode 16.4，并通过 `macos_arm64` config 把 `MACOSX_DEPLOYMENT_TARGET` 固定为 13.0，使 Apple Clang 和最低系统版本不随 runner 漂移。macOS arm64 与 Linux x86_64 对 SDL3、PortAudio 和 FFmpeg 使用相同 Git pin，并从 `third_party` source 进入 `rules_foreign_cc` action；配置、编译、安装目录和 runtime shared library 都属于 Bazel graph，不读取旧的 dependency-prefix repository environment、Homebrew prefix 或发行版 SDL、PortAudio、FFmpeg development package。coreHTTP、llhttp 与 wolfSSL 同样从固定 gitlink 静态进入 first-party target；Linux 不安装或链接发行版 libcurl。Linux 只把 X11、ALSA 等 OS service development boundary 作为显式 host dependency。Linux x86_64 CI 从 fresh output base 禁止 repository download 后重建 representative Desktop binary，并用 `tools/bazel/linux_runtime` 验证完整 `ldd` closure；allowlist 不得包含 curl、wolfSSL、OpenSSL、coreHTTP 或 llhttp。Linux aarch64 smoke 仍只验证 toolchain 生成目标架构 ELF、符号与重复构建 checksum，不承担这些 upstream 的 source build。
...
CI checkout 使用当前 event exact head，并递归初始化 Git index 指定的 submodule revision。Repository root 的 `make cfg-submodules` 提供相同的本地初始化入口；不维护第二套 committed gitlink metadata 或 affected checkout helper。

Planner 可以从相同 runner OS、architecture 和 Git major/minor 的 compatible cache 恢复 `.git/modules`。Primary key 绑定 `.gitmodules` 与当前 Git index 的 gitlink digest；stale prefix restore 只提供 object seed，不能决定当前 revision。Cache miss、eviction、corruption 或 restore/save error 回退到 shallow network fetch，不能跳过 `sync`、exact update、remote/revision/clean-state validation、Bazel analysis 或 consumer。

受保护的 `main` push 保存 default-branch seed；同仓 pull request 只允许在 miss 时保存一份由 Actions branch scope 隔离、且绑定 manifest primary key 的 seed，用于该 PR exact-head 的 warm 重跑和性能验收。Fork pull request 不保存 seed。PR seed 不能作为 `main` 或其他 PR 的 trusted writer，失效后按 repository retention/eviction 回收。

Cache archive 不包含 parent checkout、`third_party/**` worktree、LFS payload、Git config、credential、hook、index、alternates、log 或 build output。Helper 在任何 Git command 读取 restored module repository 前删除这些内容，只保留 inert object、ref 和 shallow-boundary data；再从当前 checked-in manifest 重建 detached HEAD、canonical remote 与 worktree pointer。Archive save 前重复 scrub。缓存中的 executable config、symlink、错误 remote、无效 repository 或缺少 object 只能被拒绝或丢弃，不能在 runner 上执行。

任何 required gitlink revision、remote 或 recursive status 无法 materialize 时，对应 execution class 必须失败，不能使用工作区里碰巧存在的其他 submodule HEAD。
...
增加、删除或移动 directory、dependency、ESP/BK7258/BK3633 launcher、global runner input 或 submodule gitlink 时，更新对应 BUILD target 与 compatibility，再运行：

...
同一个 wrapper 也强制注入 target-owned task policy。每个 ESP firmware target 必须显式传入 `task_policy`；每个 BK7258 firmware target 必须显式传入 `ap_task_policy` 与 `cp_task_policy`。这些参数不属于 `layout_files`，private downstream target 也遵循同一 contract。缺少参数会在 analysis 阶段以 target、board 和 field fail closed。ESP image graph 只能包含一个 `h2_esp_target_task_policy`；BK graph 对 AP/CP 各包含一个 execution-unit-specific `h2_bk_target_task_policy`，相同 component name 由 native-component execution-unit key 隔离。

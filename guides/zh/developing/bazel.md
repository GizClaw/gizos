...
同一个 wrapper 也强制注入 layout-owned task policy。ESP `layout_files` 必须提供 `task_policy`；BK7258 必须同时提供 `ap_task_policy` 与 `cp_task_policy`。repository layout 自动派生这些 labels，private downstream layout 必须显式传入。缺少字段在 analysis 阶段以 target、board 和 field fail closed。ESP image graph 只能包含一个 `h2_esp_layout_task_policy`；BK graph 对 AP/CP 各包含一个 execution-unit-specific `h2_bk_layout_task_policy`，相同 component name 由 native-component execution-unit key 隔离。

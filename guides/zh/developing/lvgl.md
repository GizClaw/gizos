...
ESP-IDF、BK7258 和 Desktop build adapter 都编译同一份 `h2_lvgl_osal.c` 与 `h2_lvgl_fs.c`。Browser 使用 `LV_OS_NONE` 保持单线程调度，但 `h2_lvgl_platform_init()` 仍复制完整 PAL 配置，并由 Web adapter 把 LVGL custom allocator 绑定到注入的 Memory PAL；调用方在 `lv_init()` 前初始化 adapter，在 `lv_deinit()` 后解除绑定。Web variant 启用 filesystem-backed TinyTTF 与 translation，并通过 `platform_web_test` 验证 allocator 和重复 lifecycle。TinyTTF 是否启用、glyph cache 容量和 translation 等 feature 由各 target 的 `lv_conf.h` 决定；这些配置不能通过修改 verified upstream archive 实现。
...
`//tools/lvgl/font_conv:font_conv.bzl` 公开 `lvgl_font` rule，并由 Bazel 固定的官方 `lv_font_conv` 生成单一字号的静态 LVGL C 字体。Consumer 提供自己的 TTF/OTF/WOFF、UTF-8 symbol source、Unicode range、字号、bpp 和 C symbol；GizOS 拥有 converter 与 Node dependency pin、host execution 和 fail-closed output validation。Rule 只收集输入文件中的非 control Unicode character，不解析 App source language 或取得 UI string ownership；生成 target 可以同时进入 `cc_library.srcs` 和 `firmware_native_component.native_srcs`。输出固定使用未压缩 bitmap，使没有启用 `LV_USE_FONT_COMPRESSED` 的 target 也能消费；需要多个字号时由 consumer 明确声明多个 target，不能在运行时调整静态字体。

`//tools/lvgl/ttf_subset:ttf_subset.bzl` 公开 `ttf_subset_c` rule，把 consumer-owned TTF 中由 UTF-8 symbol source 与 Unicode range 选中的 glyph outline 裁剪成一份 size-independent TTF，再生成只导出 data 和 size 的 portable C source/header。生成 target 可以同时进入 `cc_library.srcs` 和 `firmware_native_component.native_srcs`。Consumer 可以用同一份内存 TTF 多次调用 `lv_tiny_ttf_create_data_ex()` 创建不同字号；每个实例的 glyph cache、fallback font、生命周期和 target wiring 仍由 consumer 管理。GizOS 只拥有固定 FontTools dependency、hermetic host execution、输入验证和 deterministic output，不取得 App 菜单文字、原始字体或 fallback 文件的 ownership。

不使用 LVGL OSAL、直接把 LVGL 接到 Display PAL 的 consumer 可以复用 `libs/lvgl:single_thread_config`，它固定 `LV_OS_NONE`、libc allocator 和 RGB565。Bazel Mobile artifact 依赖 `//libs/lvgl:lvgl_mobile` 编译上游 source；Web artifact 依赖 `//libs/lvgl:lvgl_web` 由 Emscripten toolchain 编译，并通过 `//libs/lvgl:platform_web` 选择单线程 custom Memory PAL bridge。需要 target OSAL、不同 source set、compile feature 或 toolchain dependency 时才增加 `_desktop`、`_embed`、`_mobile`、`_web` variant；不能为每个 App 创建 App-named LVGL library，也不能仅为名称整齐复制相同 target。

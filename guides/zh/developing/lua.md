...
构建不给 upstream Lua 提供 libc 文件或标准流：vendor overlay 把文件 API 改名到
`//third_party/lua_patch` 中 fail-closed 的 shim，并强制包含
`h2_lua_embedded_stdio.h`，在 `<stdio.h>` 之后把 `stdin`、`stdout`、`stderr`
重定向到同一组 shim，避免 C library 通过 `_impure_ptr` 等 reentrancy 状态展开
标准流。只有 ESP32-S3/P4 额外链接 `lua_esp_libc_compat.c`，补齐 ESP-IDF picolibc

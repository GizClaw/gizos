...
它们的 `kind` 使用统一 `h2_runtime_event_kind_t` 中的 `H2_RUNTIME_COMPONENT_EVENT_*` 值。每次到达 Button source 的 poll deadline，按下状态都会产生一个 `H2_RUNTIME_COMPONENT_EVENT_BUTTON_DOWN` sample 和一个 `H2_RUNTIME_COMPONENT_EVENT_BUTTON_ACTION`：首次按下的 phase 是 `PRESSED`，持续按下的后续 poll 是 `HOLDING`。松开时先产生 `BUTTON_UP`，再产生 phase 为 `RELEASED` 的 action。Action 携带 `pressed_at_ms`、`released_at_ms`、`duration_ms` 和 `click_count`；`click_count` 按相邻点击间隔不超过 `H2_RUNTIME_BUTTON_CLICK_GAP_MS`（默认 250 ms）累计，超时后从 1 重新计数。Action cadence 就是该 source 的 poll cadence，默认 20 ms，没有额外的 100 ms timer。Runtime 只报告客观 phase 和时间，不定义 long-press、short-press 等 gesture；App 根据自己的策略选择 `PRESSED`、`HOLDING` 或 `RELEASED` 并判断时长。Button state snapshot 同时携带 `click_count`：按住期间是当前这次按压在连续点击序列中的序号，松开后保留最近一次 action 的计数，直到下一次按压延续或重置序列；首次按压前为 0。某个 proxied PAL API call 也可以按对应 component contract emit component event。
...
同理，`start` 不重置 Button action 状态，`stop` 也不丢弃已经入队但尚未消费的 push edge——它们都属于 state，不属于 poller。由此产生两个必须知道的后果：poller 关闭期间松开按键时 Runtime 看不到那个边沿，恢复后的第一帧会补发 `BUTTON_UP` 与 phase 为 `RELEASED` 的 `BUTTON_ACTION`，而 `pressed_at_ms` 仍是关闭之前那次按下的时间戳，于是该 action 的时长横跨整个关闭区间；连续点击计数也跨 stop/start 延续，`start` 之后落在 `H2_RUNTIME_BUTTON_CLICK_GAP_MS` 之内的下一次按压会继续既有序列而不是从 1 重新计数。需要忽略恢复后的第一个 action 时由调用方自行判断。

Battery 与 Temperature 通过 `h2_runtime_component_state_battery()` 和 `h2_runtime_component_state_temperature()` 返回最近一次完成的读数及 PAL result。`PUSH_EDGE` Button 不调用 Button PAL read；拥有 mapped periph 的 LVGL adapter 调用 `h2_runtime_button_push_edge()` 写入 raw edge，Runtime input task 继续推进 click gap 计数。

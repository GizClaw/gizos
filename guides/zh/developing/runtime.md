...
`h2_runtime_wifi_connect_best_saved(runtime, timeout_ms)` 读取 Runtime 集合后扫描，回调仅为每个保存 SSID 保留最强 AP，不在栈上保存完整扫描表。候选按 RSSI 降序，同强度按存储顺序排列，并钉上实际扫描到的 BSSID/channel；不可见网络不参与。依次调用 `h2_pal_wifi_sta_connect`，关联后通过 Runtime 已订阅的 station 事件唤醒条件变量，读取已发布快照，等到该 SSID 报告 GOT_IP 且地址有效才将它置顶并刷新时间戳——回写的是集合里原本存的凭据，不是钉了 BSSID 的候选，避免下次同一 SSID 换了 AP 连不上；全部失败返回最后一个连接错误，空集合或全部不可见返回 NOT_FOUND。关联后断开、失败或丢失地址时，在同一次调用内继续下一个候选，不重新扫描；每个候选的等地址窗口为 `min(H2_RUNTIME_WIFI_ADDRESS_WAIT_MS（8000 ms）, 剩余总预算)`；窗口耗尽（包括条件变量等待超时）记为该候选的 UNAVAILABLE，继续尝试下一个候选，真正的非超时错误仍原样传播。只有外层在下一次尝试前检查总预算耗尽时才返回 TIMEOUT。等待不轮询 station status，也不依赖主循环 drain。列表读取、扫描、各次连接及地址等待共用总预算，零值默认 15 秒，每次连接使用剩余预算，下一次尝试前耗尽返回 TIMEOUT；存储、扫描和单调时钟错误直接传播。没有 system event API 的平台只读一次已发布快照并沿用关联成功语义。此恢复流程不写 PAL 凭据；置顶写入失败同样只记 WARN 日志并返回 OK，集合保持不变。

`runtime->wifi_sta` 仍 1:1 暴露注入的 PAL provider。PAL Settings 始终只保存一条凭据，含义是最近一次通过持久化连接成功保存的网络；临时 connect（包括 best-saved）不会覆盖它。`h2_runtime_wifi_connect_saved()` 保持原样：读取 PAL 单条凭据，调用 connect，不重写任何存储；零 timeout 默认 15 秒，无凭据返回 NOT_FOUND。`h2_runtime_wifi_connect_saved()` 成功只保证关联完成，IP 可随后到达；`h2_runtime_wifi_connect_best_saved()` 成功表示该 SSID 已报告 GOT_IP 且 IPv4 非零。平台没有 system event API，或 provider 支持事件但不支持创建条件变量时，后者退化为只保证关联完成，Runtime 初始化不因此失败。应用负责调用时机、重试策略和所有列表/连接操作的串行化，须在 Runtime deinit 前完成操作；Runtime init 不自动联网。
...
→ h2_runtime_wait_notify() / h2_runtime_poll_event()
...
消费端仍然只使用 `h2_runtime_wait_notify()` 和 `h2_runtime_poll_event()`：
...
- 投递可以来自任意 Task，并且立即唤醒正在 `h2_runtime_wait_notify()` 中等待的消费者。

...
`PUSH_EDGE` producer 还可通过同一个 `h2_runtime_button_push_edge()` 队列投递 `H2_RUNTIME_BUTTON_EDGE_CANCEL`。Poller 按入队顺序终止当时仍然按住的 Button：先发布 `pressed=false`、`pressed_at_ms=0` 的 snapshot，再发布独立的 `H2_RUNTIME_COMPONENT_EVENT_BUTTON_CANCEL`，payload 为原始 `pressed_at_ms` 与入队时的 `cancelled_at_ms`。取消不生成 `BUTTON_UP` 或已释放的 `BUTTON_ACTION`；neutral/重复 CANCEL 和随后迟到的 UP 是 no-op，新的 DOWN 可正常开始下一次按压，已经排在 CANCEL 前面的 UP 不会被撤销。普通 held sampling 和 `POLL_STATE` 行为不变。App 保留按压期间的临时状态时应消费 CANCEL 并清理，而不是完成动作。

取消不绕过 queue、snapshot backpressure 或 source ownership。Push 成功只证明 raw edge 入队，不保证最终 callback 已执行；queue 满时不消费该 edge，producer 必须先按序重试同一个边界再接受新按压，或停止 producer/session。Test Control 的显式 event emitter 校验 cancellation payload、mapped Button 与 envelope timestamp；该 emitter 不自动修改 Button snapshot，需要 state+event fixture 的测试仍应显式先设置 neutral state。Input stop/restart 保留已接受队列，Test Control 接管则丢弃旧 source 的队列；deinit 前仍需停止 producer。

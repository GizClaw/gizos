...
唤醒必须绑定到具体 waiter：`h2_pal_cond_signal()` 至少唤醒一个调用时已注册的 waiter，`h2_pal_cond_broadcast()` 唤醒调用时已注册的全部 waiter，其他 waiter 同时 timeout 或重新进入 wait 不能吞掉属于别人的唤醒；timeout 之后、注销之前到达的唤醒仍返回 `H2_PAL_OK`；没有 waiter 时的 signal/broadcast 不留下 pending token。Backend 因此不能用共享 counting semaphore 当 token pool，必须让每个 waiter 停在自己的 semaphore 上（ESP 与 BK7258 provider 均如此实现）。

...
`h2_pal_task_options_t` 只描述任务名和最小栈大小。任务栈存储由平台 provider 和目标策略管理；ESP 可在 `h2_esp_task_policy_config_t.psram_stack_allocator` 配置共享 PSRAM allocator，仅对解析为 PSRAM 的任务生效，internal 策略保持 SDK 分配路径。

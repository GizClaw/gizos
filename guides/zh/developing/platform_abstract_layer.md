...
并发原子值由 `libs/atomic/include/h2_atomic.h` 定义，不属于 PAL API、PAL vtable 或 Memory PAL capability。调用方持有 typed wrapper 并直接调用 `h2_atomic_*` 符号；最终 target 必须链接一个平台实现，缺失实现会在链接时报错。非 flag wrapper 初始化后拥有 provider 存储，停止并发访问后销毁，不得初始化后复制；静态 flag 可以零初始化而不分配存储。Desktop/Browser 的 provider 基于 C11，iOS/Android 使用 pthread，ESP 将实际原子存储放在内部 RAM，即使 wrapper 在 PSRAM。BK/JieLi 的初始化明确返回 `H2_ATOMIC_UNSUPPORTED`，调用方必须传播错误，不能将失败后的 wrapper 当作零值使用。

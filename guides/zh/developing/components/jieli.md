...
STA 需要提取 `wpasupplicant.a` 中的真实 supplicant，覆盖 `wl_wifi.a` 的弱 stub。H2Loader layout 将 `wpasupplicant.a` 放在 `wl_wifi.a` 前，移除 `hostapd_and_wpasupplicant.a`，并通过 `LFLAGS += --undefined=wpa_supplicant_get_state` 保证提取该成员。连接前调用 SDK `wifi_set_sta_connect_timeout()`，非零 PAL 预算向上取整为秒，零预算使用 30 秒。`wifi_enter_sta_mode()` 以非阻塞方式调用，其返回值只反映站点是否已经连接，因此忽略该返回值；连接结果由 PAL 预算内的 SDK 事件决定。

...
`h2_pal_core` 只拥有一次性 policy 配置、policy 校验、FreeRTOS task 创建、诊断、join 和清理，不得比较具体 App、library、Loader、modem、board 或 product task name。最终 portable name 到 absolute priority、core affinity、minimum stack 和 Internal/PSRAM placement 的映射属于具体 firmware target，并由 target package 的 `task_policy/h2_esp_target_task_policy.h` 与共享 generic implementation 组成的 component 提供。

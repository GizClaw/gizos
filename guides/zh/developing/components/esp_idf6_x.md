...
ESP Wi-Fi 的已保存 STA 凭据与当前 driver config 分开：保存、读取和删除通过 `h2wifi/sta_v1` NVS record，运行时 STA connect 使用 `WIFI_STORAGE_RAM`。首次读取或首次运行时切网前，若 record 不存在，先导入旧 ESP-IDF STA 配置并提交；导入失败时不改变当前连接配置。删除写入 version 1 空记录，避免下次启动重新导入已删除的旧凭据。Record 使用固定 107 bytes，依次编码版本、SSID/password 长度、BSSID flag、channel、6-byte BSSID、32-byte SSID 与 64-byte password，不直接保存 SDK/C struct。NVS 访问在 Wi-Fi mutex 和 Internal SafeCall stack 内执行；保存和删除不调用 driver set-config，也不主动断开当前连接。Host adapter test 覆盖记录边界、损坏记录、读写失败和删除；NVS 首次迁移、断电恢复及实际联网仍需要 target 验证。

...
- ADC OneShot component 统一管理一个显式 ADC unit service、channel config、serialized raw/calibrated read，并提供彼此独立的 radio-button debounce、binary transition stabilization 与连续 raw ADC stabilization policy。Binary stabilizer 立即发布首个有效样本，后续状态切换需要连续两个相同的相反样本；连续 numeric stabilizer 在前五个 raw sample 中发布当前样本集的 upper median，第五个样本建立完整窗口，第六个样本开始进入慢速 EMA。Caller 只通过 OneShot stabilizer init/deinit 给具体 channel 安装、重置或移除 optional value stabilizer；不直接取得 numeric filter state，也不调用 filter update。`read_value` 在未安装时直接读取 raw count；安装且 raw threshold 为零时，每次硬件读取自动更新一次 channel filter；positive raw threshold 进一步启用 startup/jump warm-up、五点 raw reseed 和失败回滚。Jump detection 比较本次 raw trigger 与上一次成功发布的 stable raw baseline，使连续的小步变化在累计偏差达到 threshold 后也能立即 reseed。完整读取由 service mutex 串行执行。`read_raw` 与 `read_mv` 始终保持单次 direct read，Board 在 `read_value` 返回后才把 stable/immediate raw count 转换为电压或产品语义。Stabilizer state、numeric init/update、sample/wait callback 和 seed window 都是 component implementation detail，Board 只配置 channel 并消费精简 raw reading。

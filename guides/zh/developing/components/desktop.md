...
Desktop consumer 可以在标准 Memory PAL 外层维护 owner view 和 allocation generation，元数据放在 system heap，精确记录同时存活总量刷新时的 owner composition。Content sampling 必须与 allocation lifetime 同步，短时复制后在锁外分析；业务并发写入意味着内容只是一段采样窗口。Pattern tail 是未写空间的启发式估计，hash 相同只是重复候选，二者不能直接判定泄漏或允许删减 buffer。Task placeholder 不代表实际业务数据，应排除这些内容分析。

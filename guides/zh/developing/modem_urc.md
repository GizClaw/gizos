...
调用方自行保证板级操作与 provider API 按顺序调用。provider 保留原有内部递归 operation_lock，后台维护使用 try-lock，竞争时跳过本轮维护。

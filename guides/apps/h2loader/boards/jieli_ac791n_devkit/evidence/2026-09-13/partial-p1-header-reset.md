...
- Diagnostic logged `H2_JIELI_PARTIAL_P1_READY prefix=16 p2_crc=valid` repeatedly through uptime `00:35:08.670`. The normal burn timeout did not remove the partial header as in the first diagnostic attempt.
- The original host monitor terminated; the intervening startup log was not captured. A reopened UART monitor and a separate status request succeeded.
- Status reported `running_partition=1 next_partition=1 stage_valid=0 last_result=0`, with both partition identities matching the image above.

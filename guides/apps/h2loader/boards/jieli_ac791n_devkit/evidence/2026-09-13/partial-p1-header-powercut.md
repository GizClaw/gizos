...
1. `H2_JIELI_PARTIAL_P1_READY prefix=16 p2_crc=valid` through `00:39:04.470`: fixture readback confirms the first 16 P1 header bytes and erased remainder, with valid P2 CRC. The diagnostic hold survives the normal five-second writer timeout.
...
5. `H2_JIELI_BOOT_INFO result=0 base=0x4020 bytes=928957 version=26 logical=1`, followed by startup event 4/code 0 and SOFT reset reason.
6. Independent UART status succeeds: running/next partition 1, Stage invalid (cleared), last_result 0, both partition image identities equal the SHA above.

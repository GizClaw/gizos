...
Advertising configuration is staged separately from immutable in-flight command storage. Starting reserves that storage under the state gate, then submits SDK commands after unlocking. The extended parameter/data/enable descriptors and legacy payloads survive their asynchronous consumers. A competing start or stop during submission returns `WOULD_BLOCK`; data setters can prepare the next configuration without changing a borrowed payload. Stop errors preserve active/requested state. Destroy clears the staged set only after successful stop submission and retains outstanding command storage.

The existing SDK command hook now fences advertising as well as connection requests. It captures submission eligibility and generation before its consumer-thread queue query. After btstack has forwarded the commands, it queues `Q_CALLBACK` on the controller. Only that controller callback releases advertising storage. A failed fence retains storage until full host shutdown/reinitialization; it never treats a queueing error as proof of consumption. Generations fit the SDK's signed-word callback ABI and are not reset across host restarts. The SDK's full exit barriers prevent old callbacks from surviving into the next host. The hook itself retains the host, so shutdown cannot race its command submissions or deferred restart.

A legacy reconnect that arrives before retirement records a deferred restart. The controller completion wakes the SDK command loop, which checks current requested/connection/stop state before restarting. Neither queue fence, SDK call, log sink nor synchronous subscriber runs under the PAL state gate. Connection and advertising state publications use the same gate. Legacy encoding now prepares both primary and scan-response packets before publishing either; invalid UUID lists are rejected before dereference. Failed scan-response encoding preserves the complete previous configuration.

`//tools/bazel:jieli_ble_advertising_lifetime_test` extracts the actual encoders, start/stop/destroy paths and both retirement stages. The initial four cases fail under Clang/GCC against `599562f2`: AddressSanitizer catches extended stack descriptor lifetime and a NULL UUID list; delayed legacy consumption sees overwritten data; failed scan-response encoding changes the primary packet. The expanded fixture has ten failing cases and one passing pending-INIT regression guard before repair. It additionally covers held SDK submission, concurrent publication, registration/fence/stop errors and deferred restart. TSan reports the original advertising publication race. All eleven cases pass after repair with strict Clang/GCC, ASan and TSan.

The host lifecycle fixture adds a held command-hook case. Before host retention was added to that new hook, native exit observes `borrow_active` and fails under both compilers; after repair shutdown waits for it. This is an additional fail-before/pass-after refinement of the in-progress advertising implementation, not an assertion that the old connection-only hook had subscriber work. The existing connection-request fence remains tested after integrating the single shared SDK hook.

Logs: `/tmp/jieli-ble-advertising-before.log`, `/tmp/jieli-ble-advertising-gcc-before.log`, `/tmp/jieli-ble-advertising-expanded-before.log`, `/tmp/jieli-ble-advertising-gcc-expanded-before.log`, `/tmp/jieli-ble-advertising-tsan-before.log`, `/tmp/jieli-ble-advertising-tsan.log`, `/tmp/jieli-ble-advertising-gcc-after.log`, `/tmp/jieli-ble-command-retention-before.log`, `/tmp/jieli-ble-command-retention-gcc-before.log`. Hardware acceptance for these advertising changes remains pending.

...
前主动触发 panic。它用于验证未确认 APP 的 Bootloader rollback、Loader 不修改
Partition 2 状态，以及 UART/BLE coredump 流式读取；一次 panic 或重启本身不算通过。
...
Partition 2 时停留在 Loader；不得修改 Partition 2 metadata、匹配 Stage、package、
`boot_intent` 或 `last_result`，也不得继续检查、写入或启动同一 APP 形成 crash loop。
...
该历史证据采用旧的失败 Stage 清理行为。当前行为改为 rollback 后不修改 Partition 2、
Stage、package、`boot_intent` 和 `last_result`，必须重新取得对应实板证据。UART 和 BLE
coredump 流式读取证据仍为 23,264 bytes，随后 erase 和空状态复查通过。

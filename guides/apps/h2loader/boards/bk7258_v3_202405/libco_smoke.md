...
启动输出 `H2_BK_AP_BOOT image=libco-smoke`，包含 8 KiB stack 和 10,000 switch 配置。Portable scenario 完成全部 `H2_LIBCO_SMOKE_STAGE` 后只输出一次 `H2_LIBCO_SMOKE_PASS rc=0`；trial image 随后确认并输出一次 `H2_BK_LIBCO_SMOKE_READY rc=0`。任一失败输出 `H2_BK_LIBCO_SMOKE_FAIL`，不会确认 image。

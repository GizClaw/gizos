...
仓库 board identity 为 `waveshare_esp32s3_a7670e_4g`，target 和 chip 均为 `esp32s3`。Loader 与所有 App image 通过 UART0 的 CH343 USB 串口提供 H2Loader command transport，console baud 固定为 `460800`；同时启用 BLE 5 LE H2Loader transport，不支持 Bluetooth Classic。

...
Runtime 在每个按下状态的 poll 输出 button down sample 和 phased action：首次为 `PRESSED`，后续为 `HOLDING`，松开时输出 button up 和 `RELEASED` action。Action 携带 `pressed_at_ms`、`released_at_ms`、`duration_ms` 和 `click_count`；Runtime 没有 long-press event。Host 根据 game role 声明的 gesture 选择 phase，long press 由 Host 从 action 时间和 phase 推导；消费 action 的 role 不能再把同一个 sample 的 button down 重复投影为第二次 game action。Button component state 用于初始化或恢复当前快照，不需要由 Host 轮询并重新推断已经存在的 event。

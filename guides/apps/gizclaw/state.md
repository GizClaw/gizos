...
PTT 松手时冻结本轮 PCM 输入窗口：窗口非空才进入 WAITING；窗口为空则回到 IDLE。输入结束发出后本轮请求即完成，不等待服务器回复。WAITING 只是本地状态：一旦有下行音频写入 Track 就回到 IDLE，由声音接管；`H2_GIZCLAW_SESSION_WAIT_MS`（5 秒）内没有任何下行音频也静默回到 IDLE，不报错。该判断在读取快照时进行。没有“回复中”状态。按下前和松手后的 PCM 不计入本轮，按住时长不是判据。

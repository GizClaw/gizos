...
- 上行 input stream ID 与服务端下发的 stream ID 没有关系，`libs/gizclaw` 不把二者关联。上行与下行各自只有一个 active stream ID：下行音频 BOS 绑定当前下行音频流，并接受其 `:<suffix>` 子流；上一条音频流尚未结束时来了新的音频 BOS，就替换 active ID，旧流已排队和已写入 Track 的音频丢弃，旧 ID 之后的 EOS 忽略。不属于 active 下行音频流的 EOS（重复、被替换的流、从未播放的流）没有可执行的动作，错误也不例外。文字流与音频流是两条独立的流，文字事件只转发不参与状态。用我们 input ID 且带我们 label（或无音频 kind）的事件、`transcript` label 的事件描述服务端看到的上行侧（READY、识别文本、上行流的结束或拒绝）；服务端复用我们 input ID 直接回复也按下行音频流处理，没有 active 下行音频时以该 ID 结束的音频 EOS 视作无音频的本轮结束。
...
- 只有 active 下行音频流的服务端 EOS 结束本轮并产生 `REPLY_DONE`（有错误的 EOS 产生 `ERROR`）；boundary 在其 EOS 标记解码后 stage，不等待 App 侧 drain。上行流自身的 EOS、transcript 与任何 TEXT_DONE 都不结束本轮，例如翻译先完成 transcript 并关闭上行流、之后才推送译文音频；Agent workflow 可以下发多条音频流，后一条的 BOS 替换前一条，被替换流的音频丢弃、EOS 忽略，本轮只在最后一条 active 流结束时产生一次 `REPLY_DONE`。被当前 Conversation 丢弃的下行事件以 WARN 记录 stream ID、label 与 route，已接收事件保持 DEBUG。Friend / Friend Group 的 SFU Workspace 不给发言者任何下行音频流，turn 只能由本地 cancel 结束。

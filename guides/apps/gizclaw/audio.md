...
- PTT 按下（`audio_start`）置位丢包标志，并记下此刻最后一个下行音频流的 stream ID（没有则为空）。标志存在期间到达的下行 Opus 直接丢弃，不进入 ring；收到 stream ID 与记录不同的下行音频 BOS（`kind=AUDIO`）才清除标志，此后的音频照常播放。文本、转写（其 stream ID 是上一轮输入的 ID）和我们自己输入的 BOS 都不清除标志；EOS 不参与。服务器沿用同一 stream ID 或不再开新流时，这一轮一直静音。短按打断同样置位。
...
下行媒体不看 EOS，除按下后的丢包标志外收到即解码；标志只由下行音频 BOS 的 stream ID 变化清除。清空与 decoder 写入 Track 串行化：清空时持有解码锁，丢弃待解码 Opus、重置解码器并标记 Track 下行水位，旧数据不会在清空后再写入。已交给平台输出的音频缓冲不在此清空保证内。不新增 RTP payload 或时间戳格式。

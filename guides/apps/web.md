...
| `web-hardware` | `hardware` hook：`prepare` 与 `configure_runtime` 在 App 前运行、Runtime `board` 为 hook 设置的名字；Enter 经 `button` hook 推到目标自己的 periph（component 42 → periph 501），Runtime 接受该 edge 后 PASS |

/**
 * @file h2_jieli_br23_sdk_port.c
 * @brief JieLi AC695N (br23) SDK binding for the PAL core providers.
 *
...
#ifndef H2_JIELI_BR23_TASK_PRIORITY
/* Below the SDK application task (app_core priority 1..3 range); FreeRTOS
 * configMAX_PRIORITIES is 8 on br23. */
...
#define H2_JIELI_BR23_MEM_HEADER 16u

static size_t *mem_header(void *ptr)
{
    return (size_t *)((uint8_t *)ptr - H2_JIELI_BR23_MEM_HEADER);
}

void *h2_jieli_sdk_malloc(size_t size)
{
    uint8_t *block;
    if (size == 0u || size > SIZE_MAX - H2_JIELI_BR23_MEM_HEADER) {
        return NULL;
    }
    block = (uint8_t *)malloc(size + H2_JIELI_BR23_MEM_HEADER);
    if (block == NULL) {
        return NULL;
    }
    *(size_t *)block = size;
    return block + H2_JIELI_BR23_MEM_HEADER;
}
...
static int ms_to_ticks(uint32_t ms)
{
    ...
    ticks = (ms + H2_JIELI_BR23_TICK_MS - 1u) / H2_JIELI_BR23_TICK_MS;
    if (ticks == 0u) {
        ticks = 1u;
    }
...
{
    return xQueueGenericReceive((QueueHandle_t)storage, NULL, 0, pdFALSE) == pdTRUE ? 0 : 1;
}

...
static int queue_create(void *user, const h2_pal_queue_config_t *config,
  ...
  *out_queue = NULL;
  ...
  queue->storage =
      (uint8_t *)h2_jieli_sdk_malloc(config->item_size * config->item_count);
  ...
  if (queue->storage == NULL || queue->lock == NULL || queue->items == NULL ||
      queue->space == NULL) {
    queue_release(queue);
    return H2_PAL_ERR_NO_MEMORY;
  }
...
static int queue_send(void *user, h2_pal_queue_t *queue, const void *item,
                      uint32_t timeout_ms) {
  int rc;
  (void)user;
...
static int queue_send_latest(void *user, h2_pal_queue_t *queue,
                             const void *item) {
  (void)user;
  if (queue == NULL || item == NULL) {
    return H2_PAL_ERR_INVALID_ARG;
  ...
  if (queue->count == queue->capacity) {
    ...
    memcpy(slot(queue, queue->head + queue->count - 1u), item,
           queue->item_size);
    (void)h2_jieli_sdk_mutex_unlock(queue->lock);
    return H2_PAL_OK;
  }
...
static int queue_recv(void *user, h2_pal_queue_t *queue, void *out_item,
                      uint32_t timeout_ms) {
  int rc;
  (void)user;

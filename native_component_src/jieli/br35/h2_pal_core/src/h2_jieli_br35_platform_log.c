...
static size_t append(char *line, size_t capacity, size_t length,
                     const char *text) {
  size_t index = 0u;
  if (text == NULL) {
    return length;
...
static int log_write(void *user, h2_pal_log_level_t level, const char *scope,
   ...
   * CRLF fit inside H2_JIELI_BR35_LOG_LINE_MAX without truncating it. */
  char line[H2_JIELI_BR35_LOG_LINE_MAX];
  size_t length = 0u;
  (void)user;
  if (message == NULL) {

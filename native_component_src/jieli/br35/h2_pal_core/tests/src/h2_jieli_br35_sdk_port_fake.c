...
static void (*sem_wait_hook)(void);
void h2_jieli_fake_set_sem_wait_hook(void (*hook)(void)) { sem_wait_hook = hook; }
...
int h2_jieli_sdk_sem_take(h2_jieli_sdk_sem_t *sem, uint32_t timeout_ms)
{
    ...
    if (sem_wait_hook) { void (*hook)(void) = sem_wait_hook; sem_wait_hook = NULL; hook(); }
    if (sem->count > 0u) {
        sem->count--;
        return 0;
    }
...
int h2_jieli_sdk_task_delete(const char *name) { return name ? 0 : -1; }

void h2_jieli_sdk_critical_enter(void) {}
void h2_jieli_sdk_critical_exit(void) {}

...
#include "h2_jieli_br35_sdk_port_fake.h"
...
{
    const h2_pal_mem_api_t *mem = h2_jieli_br35_platform_mem_api();
    void *block;
    h2_jieli_fake_reset();
...
{
    const h2_pal_log_api_t *log = h2_jieli_br35_platform_log_api();
    h2_jieli_fake_reset();
...
static void test_log_truncates_long_messages_without_overflow(void)
{
    ...
    CHECK(length <= H2_JIELI_BR35_LOG_LINE_MAX);
    CHECK(length >= 2u);
...
{
    const h2_pal_log_api_t *log = h2_jieli_br35_platform_log_api();
    char scope[H2_JIELI_BR35_LOG_LINE_MAX];
...
{
    const h2_pal_time_api_t *time = h2_jieli_br35_platform_time_api();
    uint64_t ms = 0u;
    uint64_t us = 0u;
...
{
    const h2_pal_sync_api_t *sync = h2_jieli_br35_platform_sync_api();
...
static h2_pal_cond_t *waiting_cond;
static void signal_waiting_condition(void) {
    const h2_pal_sync_api_t *sync = h2_jieli_br35_platform_sync_api();
    CHECK(h2_pal_cond_destroy(sync, waiting_cond) == H2_PAL_ERR_BUSY);
    CHECK(h2_pal_cond_signal(sync, waiting_cond) == H2_PAL_OK);
}
static void test_condition_reacquires_mutex_and_reclaims_waiter(void) {
    const h2_pal_sync_api_t *sync = h2_jieli_br35_platform_sync_api();
    const h2_pal_mutex_config_t mc = {0};
    const h2_pal_cond_config_t cc = {0};
    h2_pal_mutex_t *mutex = NULL;
    h2_jieli_fake_reset();
    CHECK(h2_pal_mutex_create(sync, &mc, &mutex) == H2_PAL_OK);
    CHECK(h2_pal_cond_create(sync, &cc, &waiting_cond) == H2_PAL_OK);
    CHECK(h2_pal_mutex_lock(sync, mutex) == H2_PAL_OK);
    CHECK(h2_pal_cond_wait(sync, waiting_cond, mutex, 5) == H2_PAL_ERR_TIMEOUT);
    CHECK(h2_pal_mutex_try_lock(sync, mutex) == H2_PAL_ERR_WOULD_BLOCK);
    h2_jieli_fake_set_sem_wait_hook(signal_waiting_condition);
    CHECK(h2_pal_cond_wait(sync, waiting_cond, mutex, 5) == H2_PAL_OK);
    CHECK(h2_pal_mutex_try_lock(sync, mutex) == H2_PAL_ERR_WOULD_BLOCK);
    CHECK(h2_pal_mutex_unlock(sync, mutex) == H2_PAL_OK);
    CHECK(h2_pal_cond_destroy(sync, waiting_cond) == H2_PAL_OK);
    CHECK(h2_pal_mutex_destroy(sync, mutex) == H2_PAL_OK);
    CHECK(h2_jieli_fake_live_allocations() == 0);
}
...
{
    const h2_pal_queue_api_t *api = h2_jieli_br35_platform_queue_api();
...
static void test_task_start_and_join_reclaims_handle(void)
{
    const h2_pal_task_api_t *api = h2_jieli_br35_platform_task_api();
    ...
    CHECK(strncmp(h2_jieli_fake_last_task_name(), "h2_", 3) == 0);
...
{
    const h2_pal_timer_api_t *api = h2_jieli_br35_platform_timer_api();
...
{
    const h2_pal_firmware_info_api_t *api = h2_jieli_br35_platform_firmware_info_api();
...
int main(void)
{
    ...
    test_condition_reacquires_mutex_and_reclaims_waiter();
    test_queue_fifo_full_timeout_latest_and_close();
    test_task_start_and_join_reclaims_handle();

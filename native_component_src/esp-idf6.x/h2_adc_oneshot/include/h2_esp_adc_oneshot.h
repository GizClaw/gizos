 ...
 *
 * The service copies @p config and owns the resulting state. Reinitializing a
 * channel discards its previous raw filter and jump baseline. This
 * task-context API is serialized with reads and must not run from an ISR. A
 * zero raw jump threshold selects one filtered raw sample per read and requires
 * zero discard and interval fields; a positive threshold enables startup/jump
 * reseeding in raw ADC counts.
 */
 ...
 * runs its complete raw-count transaction, including configured delays, while
 * holding the service mutex. A positive threshold is measured from the last
 * successfully published stable raw baseline. On failure, channel state and
 * @p out_reading remain unchanged. This task-context API must not run from an
 * ISR. Board-specific conversion into physical units occurs after this call.
 */

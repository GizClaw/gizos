...
typedef struct h2_esp_platform_arena_config {
    ...
    const char *name;
    /* Allow requests the arena cannot serve to fall back to the system PSRAM
     * heap. Off by default: spills fragment the heap the arena is meant to
     * protect, and a loud NO_MEMORY is easier to act on. */
 ...
 */
/* The portable core behind the arena, for block inspection; NULL when
 * arena is NULL. The allocator's user pointer is not the core. */
h2_mem_arena_t *h2_esp_platform_arena_core(h2_esp_platform_arena_t *arena);

/* Logs live allocations that spilled out of the arena, grouped by caller
 * frames; at most once a minute unless force is set. */
...
/** @brief Inspect raw pool free/largest sizes at a diagnostic boundary.
 * Borrows arena; out_pools has two entries (small, large). No allocation/logging.
 * Returns INVALID_ARG for NULL arena or output. */

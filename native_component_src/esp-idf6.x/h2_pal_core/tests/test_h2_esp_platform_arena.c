...
static h2_esp_platform_arena_t *create(void) {
    const h2_esp_platform_arena_config_t config = {.reserved_bytes = RESERVATION, .name = "test", .spill_to_system = true};
...
static void test_inspection(void) {
    h2_mem_arena_pool_inspection_t before[2] = {{0}, {0}}, after[2] = {{0}, {0}};
    assert(h2_esp_platform_arena_inspect(NULL, before) == H2_PAL_ERR_INVALID_ARG);
    h2_esp_platform_arena_t *arena = create();
    assert(h2_esp_platform_arena_inspect(arena, NULL) == H2_PAL_ERR_INVALID_ARG);
    const unsigned initial_takes = takes;
    assert(h2_esp_platform_arena_inspect(arena, before) == H2_PAL_OK);
    assert(takes == initial_takes + 1u && !locked);
    const h2_pal_mem_api_t *mem = h2_esp_platform_arena_mem(arena);
    void *block = h2_pal_mem_alloc(mem, 4096u);
    assert(block != NULL);
    assert(h2_esp_platform_arena_inspect(arena, after) == H2_PAL_OK);
    assert(after[1].free_bytes < before[1].free_bytes);
    assert(after[1].largest_free_block < before[1].largest_free_block);

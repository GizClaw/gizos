...
static portMUX_TYPE s_h2_esp_wifi_safe_mutex_init_lock = portMUX_INITIALIZER_UNLOCKED;
...
static h2_pal_wifi_ap_client_t s_h2_esp_wifi_ap_clients[H2_PAL_WIFI_AP_MAX_CLIENTS];
...
static int h2_esp_wifi_sta_reconnect_claim_attempt(void) {
    ...
    if (s_h2_esp_wifi_sta_reconnect_enabled != 0 &&
        s_h2_esp_wifi_sta_reconnect_attempts < H2_ESP_WIFI_STA_RECONNECT_ATTEMPTS) {
...
static void h2_esp_wifi_post_sta_system_event(
    h2_pal_system_event_type_t type,
...
static void h2_esp_wifi_post_ap_client_system_event(
    h2_pal_system_event_type_t type,
...
static h2_pal_wifi_ap_client_t *h2_esp_wifi_find_ap_client(const uint8_t mac[6]) {
    if (mac == NULL) {
        return NULL;
    }
...
static h2_pal_wifi_ap_client_t *h2_esp_wifi_upsert_ap_client(const uint8_t mac[6]) {
...
static h2_pal_wifi_ap_client_t h2_esp_wifi_remove_ap_client(const uint8_t mac[6]) {
    ...
    for (size_t i = 0u; i < s_h2_esp_wifi_ap_client_count; ++i) {
        if (memcmp(s_h2_esp_wifi_ap_clients[i].mac, mac, 6u) == 0) {
            ...
            if (i + 1u < s_h2_esp_wifi_ap_client_count) {
                memmove(
                    &s_h2_esp_wifi_ap_clients[i],
                    &s_h2_esp_wifi_ap_clients[i + 1u],
                    (s_h2_esp_wifi_ap_client_count - i - 1u) * sizeof(s_h2_esp_wifi_ap_clients[0]));
            }
...
static int h2_esp_wifi_stop_driver_if_sta_idle(void) {
    if (s_h2_esp_wifi_events != NULL) {
        ...
        if ((bits & (H2_ESP_WIFI_EVENT_CONNECTED | H2_ESP_WIFI_EVENT_GOT_IP)) != 0u) {
            return H2_PAL_OK;
        }
    }
    ...
    if (s_h2_esp_wifi_events != NULL) {
        xEventGroupClearBits(
            ...
            H2_ESP_WIFI_EVENT_CONNECTED | H2_ESP_WIFI_EVENT_GOT_IP);
...
static void h2_esp_wifi_event_handler(
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_CONNECTED) {
        ...
        if (connected != NULL) {
            status.ssid_len = h2_esp_wifi_strnlen(connected->ssid, H2_PAL_WIFI_SSID_MAX);
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_STACONNECTED) {
        const wifi_event_ap_staconnected_t *connected = (const wifi_event_ap_staconnected_t *)event_data;
        if (connected != NULL) {
            h2_pal_wifi_ap_client_t event_client;
            memset(&event_client, 0, sizeof(event_client));
            int have_client = 0;
            portENTER_CRITICAL(&s_h2_esp_wifi_ap_lock);
            h2_pal_wifi_ap_client_t *client = h2_esp_wifi_upsert_ap_client(connected->mac);
            if (client != NULL) {
            ...
            if (have_client != 0) {
                h2_esp_wifi_post_ap_client_system_event(H2_PAL_SYSTEM_EVENT_TYPE_WIFI_AP_CLIENT_JOINED, &event_client);
            }
        }
        return;
    }
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_STADISCONNECTED) {
        const wifi_event_ap_stadisconnected_t *disconnected = (const wifi_event_ap_stadisconnected_t *)event_data;
        if (disconnected != NULL) {
            portENTER_CRITICAL(&s_h2_esp_wifi_ap_lock);
            h2_pal_wifi_ap_client_t client = h2_esp_wifi_remove_ap_client(disconnected->mac);
            ...
            s_h2_esp_wifi_ap_status.client_count = s_h2_esp_wifi_ap_client_count;
            portEXIT_CRITICAL(&s_h2_esp_wifi_ap_lock);
            h2_esp_wifi_post_ap_client_system_event(H2_PAL_SYSTEM_EVENT_TYPE_WIFI_AP_CLIENT_LEFT, &client);
            if (client.lease_valid != 0u) {
                h2_esp_wifi_post_ap_client_system_event(H2_PAL_SYSTEM_EVENT_TYPE_WIFI_AP_LEASE_RELEASED, &client);
            }
        }
        return;
    }
    if (event_base == IP_EVENT && event_id == IP_EVENT_ASSIGNED_IP_TO_CLIENT) {
        const ip_event_assigned_ip_to_client_t *assigned = (const ip_event_assigned_ip_to_client_t *)event_data;
        if (assigned != NULL) {
            h2_pal_wifi_ap_client_t event_client;
            memset(&event_client, 0, sizeof(event_client));
            int have_client = 0;
            portENTER_CRITICAL(&s_h2_esp_wifi_ap_lock);
            h2_pal_wifi_ap_client_t *client = h2_esp_wifi_upsert_ap_client(assigned->mac);
            if (client != NULL) {
            ...
            portEXIT_CRITICAL(&s_h2_esp_wifi_ap_lock);
            if (have_client != 0) {
                h2_esp_wifi_post_ap_client_system_event(H2_PAL_SYSTEM_EVENT_TYPE_WIFI_AP_LEASE_GRANTED, &event_client);
            }
        }
        return;
    }
...
int h2_esp_platform_wifi_ensure_started(void) {
    ...
    if (s_h2_esp_wifi_events_registered == 0) {
        ...
        err = esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, h2_esp_wifi_event_handler, NULL);
        if (err != ESP_OK) {
            return h2_esp_wifi_map_error(err);
        }
...
#if CONFIG_ESP_WIFI_SOFTAP_SUPPORT
        err = esp_event_handler_register(IP_EVENT, IP_EVENT_ASSIGNED_IP_TO_CLIENT, h2_esp_wifi_event_handler, NULL);
        if (err != ESP_OK) {
            return h2_esp_wifi_map_error(err);
        }
#endif
...
static void h2_esp_wifi_copy_ap_record(
...
static void h2_esp_wifi_status_from_ap(
    h2_pal_wifi_sta_status_t *out_status,
    const wifi_ap_record_t *record) {
    memset(out_status, 0, sizeof(*out_status));
    out_status->state = H2_PAL_WIFI_STA_STATE_CONNECTED;
    out_status->ssid_len = h2_esp_wifi_strnlen(record->ssid, H2_PAL_WIFI_SSID_MAX);
...
static void h2_esp_wifi_copy_sta_config(
...
static void h2_esp_wifi_copy_ap_config(
    wifi_config_t *out_config,
    ...
    out_config->ap.max_connection = config->max_clients != 0u ? config->max_clients : H2_PAL_WIFI_AP_MAX_CLIENTS;
...
static void h2_esp_wifi_set_ap_status_from_config(const h2_pal_wifi_ap_config_t *config) {
    ...
    s_h2_esp_wifi_ap_status.channel = config->channel != 0u ? config->channel : 1u;
    s_h2_esp_wifi_ap_status.max_clients = config->max_clients != 0u ? config->max_clients : H2_PAL_WIFI_AP_MAX_CLIENTS;
...
static int h2_esp_wifi_ap_config_same(const h2_pal_wifi_ap_config_t *config) {
    ...
    int same = s_h2_esp_wifi_ap_active != 0 &&
        ...
        memcmp(s_h2_esp_wifi_ap_config.ssid, config->ssid, config->ssid_len) == 0 &&
        memcmp(s_h2_esp_wifi_ap_config.password, config->password, config->password_len) == 0;
    portEXIT_CRITICAL(&s_h2_esp_wifi_ap_lock);
    return same;
}
#endif

static int h2_esp_wifi_sta_get_status(
    ...
    if (err == ESP_OK) {
        out_status->ssid_len = h2_esp_wifi_strnlen(config.sta.ssid, H2_PAL_WIFI_SSID_MAX);
...
static int h2_esp_wifi_sta_scan(
    h2_pal_wifi_sta_t *sta,
...
static int h2_esp_wifi_sta_connect(
    h2_pal_wifi_sta_t *sta,
    const h2_pal_wifi_sta_config_t *config,
    uint32_t timeout_ms) {
    (void)sta;
    ...
    if (was_connected != 0) {
        xEventGroupClearBits(
            s_h2_esp_wifi_events,
            H2_ESP_WIFI_EVENT_CONNECTED | H2_ESP_WIFI_EVENT_DISCONNECTED | H2_ESP_WIFI_EVENT_GOT_IP);
        err = esp_wifi_disconnect();
        if (err != ESP_OK && err != ESP_ERR_WIFI_NOT_CONNECT && err != ESP_ERR_WIFI_NOT_ASSOC) {
            h2_esp_wifi_set_activity(0);
            return h2_esp_wifi_map_error(err);
        }
    ...
    h2_esp_wifi_post_sta_system_event(H2_PAL_SYSTEM_EVENT_TYPE_WIFI_STA_CONNECTING, &connecting_status);
    ...
    for (;;) {
        ...
        EventBits_t bits = xEventGroupWaitBits(
            s_h2_esp_wifi_events,
            H2_ESP_WIFI_EVENT_CONNECTED | H2_ESP_WIFI_EVENT_DISCONNECTED,
            pdFALSE,
            pdFALSE,
            remaining);
        if ((bits & H2_ESP_WIFI_EVENT_CONNECTED) != 0u) {
            ...
            memset(&ap_info, 0, sizeof(ap_info));
            if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
                size_t ssid_len = h2_esp_wifi_strnlen(
                    ap_info.ssid, H2_PAL_WIFI_SSID_MAX);
        ...
        if ((bits & H2_ESP_WIFI_EVENT_DISCONNECTED) == 0u ||
            !h2_esp_wifi_sta_reconnect_is_enabled() ||
            remaining == 0u) {
            break;
        }
        xEventGroupClearBits(
            s_h2_esp_wifi_events, H2_ESP_WIFI_EVENT_DISCONNECTED);
    }
...
#if CONFIG_ESP_WIFI_SOFTAP_SUPPORT
static int h2_esp_wifi_ap_start(
    h2_pal_wifi_ap_t *ap,
    const h2_pal_wifi_ap_config_t *config,
    uint32_t timeout_ms) {
    (void)ap;
    ...
    portEXIT_CRITICAL(&s_h2_esp_wifi_ap_lock);
    h2_esp_wifi_post_ap_system_event(H2_PAL_SYSTEM_EVENT_TYPE_WIFI_AP_STARTED, &status);
    return H2_PAL_OK;
}
...
static int h2_esp_wifi_ap_stop(h2_pal_wifi_ap_t *ap, uint32_t timeout_ms) {
    ...
    h2_esp_wifi_post_ap_system_event(H2_PAL_SYSTEM_EVENT_TYPE_WIFI_AP_STOPPED, &status);
    return h2_esp_wifi_stop_driver_if_sta_idle();
}

static int h2_esp_wifi_ap_get_status(
    h2_pal_wifi_ap_t *ap,
...
static int h2_esp_wifi_ap_get_clients(
    h2_pal_wifi_ap_t *ap,
    ...
    if (copy_count > 0u) {
        memcpy(out_clients, s_h2_esp_wifi_ap_clients, copy_count * sizeof(out_clients[0]));
    }
...
static h2_atomic_flag_t s_h2_esp_wifi_connect_busy;
static bool s_h2_esp_wifi_connect_busy_ready;

h2_pal_result_t h2_esp_platform_wifi_atomic_init(void) {
    if (s_h2_esp_wifi_connect_busy_ready) return H2_PAL_ERR_INVALID_STATE;
    const h2_atomic_result_t rc =
        h2_atomic_flag_init(&s_h2_esp_wifi_connect_busy);
    if (rc == H2_ATOMIC_UNSUPPORTED) return H2_PAL_ERR_UNSUPPORTED;
    if (rc != H2_ATOMIC_OK) return H2_PAL_ERR_NO_MEMORY;
    s_h2_esp_wifi_connect_busy_ready = true;
    return H2_PAL_OK;
}

h2_pal_result_t h2_esp_platform_wifi_atomic_can_shutdown(void) {
    if (!s_h2_esp_wifi_connect_busy_ready) return H2_PAL_ERR_INVALID_STATE;
    if (h2_atomic_flag_test_and_set(&s_h2_esp_wifi_connect_busy,
                                    H2_ATOMIC_ACQUIRE)) return H2_PAL_ERR_BUSY;
    h2_atomic_flag_clear(&s_h2_esp_wifi_connect_busy, H2_ATOMIC_RELEASE);
    return H2_PAL_OK;
}

h2_pal_result_t h2_esp_platform_wifi_atomic_shutdown(void) {
    h2_pal_result_t rc = h2_esp_platform_wifi_atomic_can_shutdown();
    if (rc != H2_PAL_OK) return rc;
    s_h2_esp_wifi_connect_busy_ready = false;
    h2_atomic_flag_destroy(&s_h2_esp_wifi_connect_busy);
    return H2_PAL_OK;
}
...
int h2_esp_platform_wifi_connect_saved(uint32_t timeout_ms) {
    ...
    TickType_t elapsed = xTaskGetTickCount() - started_at;
    TickType_t remaining = timeout_ms == 0u || elapsed < timeout_ticks ? timeout_ticks - elapsed : 0u;
    EventBits_t bits = xEventGroupWaitBits(
        s_h2_esp_wifi_events,
        H2_ESP_WIFI_EVENT_GOT_IP | H2_ESP_WIFI_EVENT_DISCONNECTED,
        pdFALSE,
        pdFALSE,
        remaining);
    if ((bits & H2_ESP_WIFI_EVENT_GOT_IP) != 0u) {

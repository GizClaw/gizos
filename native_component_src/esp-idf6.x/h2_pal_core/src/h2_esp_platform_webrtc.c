...
static bool h2_esp_h2peer_lock_ready;

h2_pal_result_t h2_esp_platform_webrtc_atomic_init(void) {
    if (h2_esp_h2peer_lock_ready) return H2_PAL_ERR_INVALID_STATE;
    h2_pal_result_t rc = h2_peer_global_init();
    if (rc != H2_PAL_OK) return rc;
    const h2_atomic_result_t atomic_rc = h2_atomic_flag_init(&h2_esp_h2peer_lock);
    ...
    h2_esp_h2peer_lock_ready = true;
    return H2_PAL_OK;
}

h2_pal_result_t h2_esp_platform_webrtc_atomic_shutdown(void) {
    if (!h2_esp_h2peer_lock_ready) return H2_PAL_ERR_INVALID_STATE;
    if (h2_atomic_flag_test_and_set(&h2_esp_h2peer_lock, H2_ATOMIC_ACQUIRE))
        return H2_PAL_ERR_BUSY;
    h2_peer_destroy(&h2_esp_h2peer);
    if (h2_esp_h2peer != NULL) {
    ...
    h2_esp_h2peer_initialized = 0;
    h2_esp_h2peer_lock_ready = false;
    h2_atomic_flag_destroy(&h2_esp_h2peer_lock);
    return H2_PAL_OK;
}
...
const h2_pal_webrtc_api_t *h2_esp_platform_webrtc_api(void) {
    if (!h2_esp_h2peer_lock_ready) return h2_pal_unsupported_webrtc_api();

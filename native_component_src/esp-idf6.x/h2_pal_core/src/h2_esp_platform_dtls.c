...
#define H2_ESP_DTLS_CERT_CAPACITY 1024u
#define H2_ESP_DTLS_SRTP_EXPORTER_LABEL "EXTRACTOR-dtls_srtp"

static const uint8_t h2_esp_dtls_transport_id[] = "h2peer";

static psa_algorithm_t h2_esp_dtls_ecdsa_algorithm(void) {
    /*
     * Mbed TLS v4 exposes its PK/TLS signing choice as a PSA algorithm. The
     * selector expands to deterministic or randomized ECDSA to match
     * mbedtls_pk_sign(), so the generated key policy must use the same value.
     */
    return (psa_algorithm_t)MBEDTLS_PK_ALG_ECDSA(PSA_ALG_SHA_256);
}
...
struct h2_pal_dtls_session {
    ...
    mbedtls_svc_key_id_t private_key_id;
    uint8_t certificate_der[H2_ESP_DTLS_CERT_CAPACITY];
    ...
    h2_esp_dtls_captured_state_t captured;
    uint8_t *plaintext;
    const uint8_t *input;
    size_t input_len;
...
static void h2_esp_dtls_set_input(
...
static void h2_esp_dtls_clear_input(h2_pal_dtls_session_t *session) {
    session->input = NULL;
    session->input_len = 0u;
    session->input_consumed = 0;
}

static void h2_esp_dtls_timer_set(
...
static int h2_esp_dtls_timer_get(void *user) {
    h2_pal_dtls_session_t *session = (h2_pal_dtls_session_t *)user;
...
static int h2_esp_dtls_defer_verify(
    ...
    if (depth == 0) {
        ...
        if (psa_hash_compute(
                PSA_ALG_SHA_256, certificate->raw.p, certificate->raw.len,
                fingerprint, sizeof(fingerprint), &hash_len) != PSA_SUCCESS ||
            h2_esp_dtls_captured_state_set_peer(
                &session->captured, fingerprint, hash_len) != H2_PAL_OK) {
...
static void h2_esp_dtls_export_keys(
    void *user,
    ...
    (void)h2_esp_dtls_captured_state_set_exporter(
        &session->captured, secret, secret_len, client_random, server_random,
        (int)prf);
}

static h2_pal_result_t h2_esp_dtls_generate_identity(
    h2_pal_dtls_session_t *session) {
    ...
    psa_set_key_algorithm(&attributes, h2_esp_dtls_ecdsa_algorithm());
    psa_result = psa_generate_key(&attributes, &session->private_key_id);
    psa_reset_key_attributes(&attributes);
    if (psa_result != PSA_SUCCESS ||
        mbedtls_pk_wrap_psa(
            &session->private_key, session->private_key_id) != 0) {
        return psa_result == PSA_ERROR_INSUFFICIENT_MEMORY
    ...
    if (result == 0) {
        /* Mbed TLS 4 keeps the RNG in the writer and uses the 3-argument API. */
    ...
    size_t hash_len = 0u;
    psa_result = psa_hash_compute(
        PSA_ALG_SHA_256, certificate, certificate_len,
        session->local_fingerprint, sizeof(session->local_fingerprint),
        &hash_len);
    return psa_result == PSA_SUCCESS &&
                   hash_len == sizeof(session->local_fingerprint)
               ? H2_PAL_OK
               : H2_PAL_ERR_IO;
}

static void h2_esp_dtls_destroy_impl(h2_pal_dtls_session_t *session) {
    if (session == NULL) {
        return;
    }
    ...
    h2_pal_mem_free(h2_esp_platform_default_allocator(), session->plaintext);
    if (!mbedtls_svc_key_id_is_null(session->private_key_id)) {
        (void)psa_destroy_key(session->private_key_id);
    }
    memset(session, 0, sizeof(*session));
    h2_pal_mem_free(h2_esp_platform_default_allocator(), session);
}

static h2_pal_result_t h2_esp_dtls_create(
    void *user,
    const h2_pal_dtls_session_config_t *config,
    h2_pal_dtls_session_t **out_session) {
    (void)user;
    h2_pal_dtls_session_t *session = h2_pal_mem_alloc(
        h2_esp_platform_default_allocator(), sizeof(*session));
    if (session == NULL) {
    ...
    session->plaintext = h2_pal_mem_alloc(
        h2_esp_platform_default_allocator(), config->max_plaintext_size);
    if (session->plaintext == NULL) {
        h2_esp_dtls_destroy_impl(session);
        return H2_PAL_ERR_NO_MEMORY;
    }
    session->private_key_id = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    h2_pal_result_t identity_result = h2_esp_dtls_generate_identity(session);
    ...
    if (result == 0) {
        /* WebRTC authenticates the peer certificate against the SDP
         * fingerprint after the handshake. OPTIONAL still requests a client
         * certificate without requiring an unrelated X.509 CA chain. */
        mbedtls_ssl_conf_authmode(
            &session->ssl_config, MBEDTLS_SSL_VERIFY_OPTIONAL);
        mbedtls_ssl_conf_verify(
            &session->ssl_config, h2_esp_dtls_defer_verify, session);
        mbedtls_ssl_conf_ciphersuites(
            &session->ssl_config, cipher_suites);
    ...
    if (result == 0 && config->role == H2_PAL_DTLS_ROLE_SERVER) {
        /* Mbed TLS 4 configures cookie entropy internally. */
        result = mbedtls_ssl_cookie_setup(&session->cookie);
        if (result == 0) {
            session->cookie_ready = 1;
    ...
    if (result == 0) {
        mbedtls_ssl_set_bio(
            &session->ssl, session,
            h2_esp_dtls_send, h2_esp_dtls_recv, NULL);
        mbedtls_ssl_set_timer_cb(
            &session->ssl, session,
            h2_esp_dtls_timer_set, h2_esp_dtls_timer_get);
        mbedtls_ssl_set_export_keys_cb(
            &session->ssl, h2_esp_dtls_export_keys, session);
    }
    if (result == 0 && config->role == H2_PAL_DTLS_ROLE_SERVER) {
        result = mbedtls_ssl_set_client_transport_id(
            &session->ssl, h2_esp_dtls_transport_id,
            sizeof(h2_esp_dtls_transport_id) - 1u);
    }
    if (result != 0) {
        h2_esp_dtls_destroy_impl(session);
...
static h2_pal_result_t h2_esp_dtls_local_fingerprint(
    void *user,
    h2_pal_dtls_session_t *session,
...
static h2_pal_result_t h2_esp_dtls_remote_fingerprint(
    void *user,
    h2_pal_dtls_session_t *session,
...
static h2_pal_result_t h2_esp_dtls_verify_peer(
    h2_pal_dtls_session_t *session) {
    return h2_esp_dtls_captured_state_verify_peer(
        &session->captured, session->remote_fingerprint);
}

static h2_pal_result_t h2_esp_dtls_handshake(
    void *user,
    h2_pal_dtls_session_t *session,
    const uint8_t *datagram,
    size_t datagram_len,
    ...
    h2_esp_dtls_set_input(session, datagram, datagram_len);
    int result = mbedtls_ssl_handshake(&session->ssl);
    h2_esp_dtls_clear_input(session);
    if (result == 0) {
        h2_pal_result_t verify_result = h2_esp_dtls_verify_peer(session);
    ...
    if (result == MBEDTLS_ERR_SSL_HELLO_VERIFY_REQUIRED &&
        session->config.role == H2_PAL_DTLS_ROLE_SERVER) {
        h2_esp_dtls_captured_state_reset_peer(&session->captured);
        ...
        return result == 0 ? H2_PAL_ERR_WOULD_BLOCK : H2_PAL_ERR_IO;
    }
...
static h2_pal_result_t h2_esp_dtls_next_deadline(
    void *user,
    h2_pal_dtls_session_t *session,
...
static h2_pal_result_t h2_esp_dtls_flush(
...
static h2_pal_result_t h2_esp_dtls_profile(
    void *user,
    h2_pal_dtls_session_t *session,
    ...
    if (info.chosen_dtls_srtp_profile !=
        MBEDTLS_TLS_SRTP_AES128_CM_HMAC_SHA1_80) {
...
static h2_pal_result_t h2_esp_dtls_export(
    void *user,
    h2_pal_dtls_session_t *session,
    uint8_t *out,
    size_t out_len) {
    (void)user;
    if (!session->handshake_complete || !session->captured.exporter_ready) {
        return H2_PAL_ERR_INVALID_STATE;
    }
    return mbedtls_ssl_tls_prf(
               (mbedtls_tls_prf_types)session->captured.prf,
               session->captured.master_secret,
               session->captured.master_secret_len,
               H2_ESP_DTLS_SRTP_EXPORTER_LABEL,
               session->captured.randoms, sizeof(session->captured.randoms),
               out, out_len) == 0
               ? H2_PAL_OK
               : H2_PAL_ERR_IO;
}

static h2_pal_result_t h2_esp_dtls_write(
    void *user,
    h2_pal_dtls_session_t *session,
    const uint8_t *data,
    size_t data_len) {
...
static h2_pal_result_t h2_esp_dtls_consume_datagram(
    void *user,
    h2_pal_dtls_session_t *session,
    const uint8_t *datagram,
    size_t datagram_len) {
...
static h2_pal_result_t h2_esp_dtls_close(
    void *user, h2_pal_dtls_session_t *session) {
    (void)user;
...
static void h2_esp_dtls_destroy(
    void *user, h2_pal_dtls_session_t **session) {
    (void)user;
    h2_pal_dtls_session_t *owned = *session;
    *session = NULL;
    h2_esp_dtls_destroy_impl(owned);
}

static const h2_pal_dtls_vtable_t h2_esp_dtls_vtable = {
    .session_create = h2_esp_dtls_create,
    .session_get_local_fingerprint = h2_esp_dtls_local_fingerprint,
    .session_set_remote_fingerprint = h2_esp_dtls_remote_fingerprint,
    .session_handshake = h2_esp_dtls_handshake,
    .session_next_deadline_ms = h2_esp_dtls_next_deadline,
    .session_flush = h2_esp_dtls_flush,
    .session_get_srtp_profile = h2_esp_dtls_profile,
    .session_export_srtp_keying_material = h2_esp_dtls_export,
    .session_write = h2_esp_dtls_write,
    .session_consume_datagram = h2_esp_dtls_consume_datagram,
    .session_close = h2_esp_dtls_close,
    .session_destroy = h2_esp_dtls_destroy,
};

const h2_pal_dtls_api_t *h2_esp_platform_dtls_api(void) {
    static const h2_pal_dtls_api_t api = {
        .user = NULL,
        .vtable = &h2_esp_dtls_vtable,
    };
    return &api;
}

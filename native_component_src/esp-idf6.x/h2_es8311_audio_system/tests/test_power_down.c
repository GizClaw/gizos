...
#include "../src/h2_esp_es8311_platform_audio.c"
...
int gpio_output_disable(gpio_num_t pin) { (void)pin; return ESP_OK; }
...
void vTaskDelay(TickType_t delay) { ticks += delay; if (running && join_workers) { if (!running->mic_task_started) running->mic_task = NULL; if (!running->playback_started) running->playback_task = NULL; } }
...
int h2_esp_es8311_sr_init(h2_esp_es8311_sr_state_t *s, const h2_esp_es8311_audio_system_config_t *c) { (void)c; s->initialized = 1; return H2_AUDIO_OK; }
void h2_esp_es8311_sr_deinit(h2_esp_es8311_sr_state_t *s) { s->initialized = 0; }
void h2_esp_es8311_sr_reset(h2_esp_es8311_sr_state_t *s) { (void)s; }
int h2_esp_es8311_sr_process(h2_esp_es8311_sr_state_t *s, const h2_audio_frame_t *r, h2_audio_frame_t *o, uint32_t t) { (void)s; (void)r; (void)o; (void)t; return H2_AUDIO_ERR_UNSUPPORTED; }

static const h2_pal_queue_api_t queue_api = {0};
static const h2_pal_sync_api_t sync_api = {0};
static h2_esp_es8311_audio_system_config_t config(void) {
    return (h2_esp_es8311_audio_system_config_t){
        .sample_rate_hz = 16000, .frame_samples_per_channel = 160,
        .raw_channels = 2, .processed_channels = 1,
        .mic_channel_index = 1, .ref_channel_index = 0,
        .mclk_multiple = 384, .pa_gpio = -1, .pa_set = fake_pa,
        .codec_i2c_addr = 0x18, .adc_digital_volume = 0xbf,
...
int main(void) {
    ...
    for (int i = 0; i < 15; ++i) {
        ...
        assert(h2_esp_es8311_audio_system_power_down(&s) == H2_AUDIO_ERR_IO);
        assert_sequence(); assert(removed == 0 && disabled == 0);
        assert(s.codec && s.codec_shutdown_pending);
        assert(es8311_audio_open(&s) == H2_AUDIO_ERR_INVALID_STATE);
    ...
    pa_fail = 0;
    assert(h2_esp_es8311_audio_system_power_down(&s) == H2_AUDIO_OK);
    open_system(&s); s.mic_started = 1; s.mic_task_started = 1; s.mic_task = (void *)7; join_workers = 0;
    assert(h2_esp_es8311_audio_system_power_down(&s) != H2_AUDIO_OK);
    assert(write_count == 0 && removed == 0);
    join_workers = 1;
    assert(h2_esp_es8311_audio_system_power_down(&s) == H2_AUDIO_OK);
    open_system(&s);
    assert(es8311_audio_start_speaker(&s) == H2_AUDIO_OK);
    write_count = 0;
    assert(es8311_audio_stop_speaker(&s) == H2_AUDIO_OK);
    assert(write_count == 0 && s.codec);
    assert(h2_esp_es8311_audio_system_power_down(&s) == H2_AUDIO_OK);
    assert_sequence();
    open_system(&s);
    assert(es8311_audio_start_speaker(&s) == H2_AUDIO_OK);
    write_count = 0; join_workers = 0;
    assert(h2_esp_es8311_audio_system_power_down(&s) == H2_AUDIO_ERR_WOULD_BLOCK);

...
h2_pal_result_t h2_bk_target_task_policy_install(void) {
  ...
  const h2_pal_result_t rc = h2_bk_platform_task_configure(&config);
  if (rc == H2_PAL_OK) {
    printf("H2_PAL_TASK_POLICY_READY unit=cp target=%s\n",
           H2_TARGET_TASK_POLICY_NAME);
  } else {
    printf("H2_PAL_TASK_POLICY_FAIL unit=cp target=%s stage=configure "
           "reason=%d\n",

...
typedef struct h2_bk_platform_crypto_backend {
    const h2_pal_mem_api_t *allocator;
    void *random_user;
    h2_pal_result_t (*random)(void *user, uint8_t *out, size_t len);
} h2_bk_platform_crypto_backend_t;

static h2_pal_result_t h2_bk_crypto_random(void *user, uint8_t *out, size_t len) {
...
static h2_bk_platform_crypto_backend_t *h2_crypto_backend(void *user) {
    return (h2_bk_platform_crypto_backend_t *)user;
}

static h2_pal_result_t h2_mbedtls_crypto_random(void *user, uint8_t *out, size_t len) {
    h2_bk_platform_crypto_backend_t *backend = h2_crypto_backend(user);
    if (out == NULL && len > 0u) {
        return H2_PAL_ERR_INVALID_ARG;
    }
...
static int h2_crypto_prepare_out(
    h2_bk_platform_crypto_backend_t *backend,
...
static void h2_bk_platform_crypto_init_api(
...
const h2_pal_crypto_api_t *h2_bk_platform_crypto_api(void) {
    static h2_bk_platform_crypto_backend_t backend;
    static h2_pal_crypto_api_t api;
    static int initialized;
    if (!initialized) {
        backend.allocator = h2_bk_platform_default_allocator();
        backend.random_user = NULL;
        backend.random = h2_bk_crypto_random;
        h2_bk_platform_crypto_init_api(&api, &backend);
        initialized = 1;
    }
    return &api;
}

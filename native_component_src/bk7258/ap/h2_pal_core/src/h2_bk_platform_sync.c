...
static h2_pal_result_t bk_mutex_create(
    ...
    *out_mutex = NULL;
    ...
    mutex->handle = mutex->recursive
        ? (beken_mutex_t)xSemaphoreCreateRecursiveMutexStatic(&mutex->storage)
        : (beken_mutex_t)xSemaphoreCreateMutexStatic(&mutex->storage);
...
static h2_pal_result_t bk_mutex_try_lock(void *user, h2_pal_mutex_t *mutex) {
    ...
    if (mutex->recursive) {
        BaseType_t ret = xSemaphoreTakeRecursive(
            (SemaphoreHandle_t)mutex->handle,
            0u);
        return ret == pdPASS ? H2_PAL_OK : H2_PAL_ERR_WOULD_BLOCK;
    }
    return rtos_trylock_mutex(&mutex->handle) == kNoErr ?
        H2_PAL_OK : H2_PAL_ERR_WOULD_BLOCK;
}
...
static h2_pal_result_t bk_semaphore_create(
    ...
    *out_semaphore = NULL;
    ...
    semaphore->handle = (beken_semaphore_t)xSemaphoreCreateCountingStatic(
        (UBaseType_t)config->max_count,
        (UBaseType_t)config->initial_count,
        &semaphore->storage);

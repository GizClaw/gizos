#include "h2_pixa_games_dinodive.h"
...
#define H2_PIXA_GAMES_DINODIVE_WIDTH 240
#define H2_PIXA_GAMES_DINODIVE_HEIGHT 240
#define H2_PIXA_GAMES_DINODIVE_FRAME_MS 17u
#define H2_PIXA_GAMES_DINODIVE_MAX_FRAME_LAG_MS 68u

typedef struct h2_pixa_games_dinodive_state {
    ...
    int display_opened;
    int speaker_started;
} h2_pixa_games_dinodive_state_t;

static h2_dinodive_t *s_active_game;
...
static void game_input(
    ...
    if (s_active_game != NULL) {
        h2_dinodive_handle_input(s_active_game, event);
    }
}
...
static int handle_runtime_event(
    ...
    if (event->component != H2_RUNTIME_COMPONENT_BUTTON ||
        ...
        !h2_runtime_button_action_is_pressed(event->payload)) {
        return H2_GAME_RUNTIME_OK;
    }
    if (event->component_id == H2_PIXA_GAMES_DINODIVE_COMPONENT_LEFT) {
        return send_input(
            state, H2_DINODIVE_BUTTON_LEFT, (uint32_t)event->timestamp_ms);
    }
    if (event->component_id == H2_PIXA_GAMES_DINODIVE_COMPONENT_RIGHT) {
        return send_input(
            state, H2_DINODIVE_BUTTON_RIGHT, (uint32_t)event->timestamp_ms);
    }
    if (event->component_id != H2_PIXA_GAMES_DINODIVE_COMPONENT_RECORD) {
        return H2_GAME_RUNTIME_OK;
    }

    h2_dinodive_result_t result = {0};
    int rc = h2_dinodive_get_result(state->game, &result);
    if (rc != H2_DINODIVE_OK) {
        return rc;
    }
    if (result.game_over) {
        h2_dinodive_reset(state->game);
    }
    return send_input(
        state, H2_DINODIVE_BUTTON_ACTION, (uint32_t)event->timestamp_ms);
}

static int cleanup(h2_pixa_games_dinodive_state_t *state, int rc) {
    s_active_game = NULL;
    h2_game_runtime_destroy(state->game_runtime);
    h2_dinodive_destroy(state->game);
    if (state->audio != NULL) {
...
int h2_pixa_games_dinodive_run(
    ...
    if (rc != H2_DISPLAY_OK ||
        display_info.width != H2_PIXA_GAMES_DINODIVE_WIDTH ||
        display_info.height != H2_PIXA_GAMES_DINODIVE_HEIGHT) {
        return cleanup(
            &state,
    ...
    const h2_dinodive_config_t game_config = {
        .player = &state.player_asset,
        .audio = state.audio,
        .text = &text,
        .texts = h2_dinodive_english_texts(),
        .seed = config->seed,
        .event_callback = NULL,
        .event_user = NULL,
    };
    rc = h2_dinodive_create(&game_config, &state.game);
    if (rc != H2_DINODIVE_OK) {
        return cleanup(&state, rc);
    }
    ...
    const h2_game_runtime_config_t game_runtime_config = {
        ...
        .width = H2_PIXA_GAMES_DINODIVE_WIDTH,
        .height = H2_PIXA_GAMES_DINODIVE_HEIGHT,
        .scene = h2_dinodive_scene(state.game),
    ...
    for (;;) {
        ...
        if (now_ms >= next_frame_ms) {
            ...
            next_frame_ms += H2_PIXA_GAMES_DINODIVE_FRAME_MS;
            if (now_ms > next_frame_ms &&
                now_ms - next_frame_ms >
                H2_PIXA_GAMES_DINODIVE_MAX_FRAME_LAG_MS) {
                next_frame_ms = now_ms + H2_PIXA_GAMES_DINODIVE_FRAME_MS;
            }
        }

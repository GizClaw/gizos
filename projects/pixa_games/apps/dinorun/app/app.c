#include "h2_pixa_games_dinorun.h"
...
#define H2_PIXA_GAMES_DINORUN_WIDTH 240
#define H2_PIXA_GAMES_DINORUN_HEIGHT 240
#define H2_PIXA_GAMES_DINORUN_FRAME_MS 17u
#define H2_PIXA_GAMES_DINORUN_MAX_FRAME_LAG_MS 68u

typedef struct h2_pixa_games_dinorun_state {
    ...
    int display_opened;
    int speaker_started;
} h2_pixa_games_dinorun_state_t;

static h2_dinorun_t *s_active_game;

static int load_player(
    h2_pixa_games_dinorun_state_t *state,
    const char *path) {
...
static void game_input(
    ...
    if (s_active_game != NULL) {
        h2_dinorun_handle_input(s_active_game, event);
    }
}
...
static int send_input(
    ...
    const h2_game_input_event_t input = {
        ...
        .x = 0,
        .y = 0,
        .button = H2_DINORUN_BUTTON_ACTION,
    };
...
static int handle_runtime_event(
    ...
    h2_dinorun_result_t result = {0};
    int rc = h2_dinorun_get_result(state->game, &result);
    if (rc != H2_DINORUN_OK) {
        return rc;
    }
    if (result.game_over) {
        h2_dinorun_reset(state->game);
    }
    return send_input(
        state,
        H2_GAME_INPUT_BUTTON_DOWN,
        (uint32_t)event->timestamp_ms);
}

static int cleanup(h2_pixa_games_dinorun_state_t *state, int rc) {
    s_active_game = NULL;
    h2_game_runtime_destroy(state->game_runtime);
    h2_dinorun_destroy(state->game);
    if (state->audio != NULL) {
...
int h2_pixa_games_dinorun_run(
    ...
    if (rc != H2_DISPLAY_OK ||
        display_info.width != H2_PIXA_GAMES_DINORUN_WIDTH ||
        display_info.height != H2_PIXA_GAMES_DINORUN_HEIGHT) {
        return cleanup(
            &state,
    ...
    const h2_dinorun_config_t game_config = {
        .player = &state.player_asset,
        .audio = state.audio,
        .text = &text,
        .texts = h2_dinorun_english_texts(),
        .seed = config->seed,
        .event_callback = NULL,
        .event_user = NULL,
    };
    rc = h2_dinorun_create(&game_config, &state.game);
    if (rc != H2_DINORUN_OK) {
        return cleanup(&state, rc);
    }
    ...
    const h2_game_runtime_config_t game_runtime_config = {
        ...
        .width = H2_PIXA_GAMES_DINORUN_WIDTH,
        .height = H2_PIXA_GAMES_DINORUN_HEIGHT,
        .scene = h2_dinorun_scene(state.game),
    ...
    for (;;) {
        ...
        if (now_ms >= next_frame_ms) {
            ...
            next_frame_ms += H2_PIXA_GAMES_DINORUN_FRAME_MS;
            if (now_ms > next_frame_ms &&
                now_ms - next_frame_ms >
                H2_PIXA_GAMES_DINORUN_MAX_FRAME_LAG_MS) {
                next_frame_ms = now_ms + H2_PIXA_GAMES_DINORUN_FRAME_MS;
            }
        }

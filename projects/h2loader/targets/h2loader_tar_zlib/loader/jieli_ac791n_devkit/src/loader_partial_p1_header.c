...
extern int decode_data_by_user_key(uint16_t, uint8_t *, uint16_t,
                                   uint32_t, uint8_t);
static int armed;
static h2_atomic_bool_t paused;
static int paused_initialized;

int h2_jieli_loader_powercut_paused(void) {
  return paused_initialized && h2_atomic_bool_load(&paused, H2_ATOMIC_ACQUIRE);
}
...
void h2_jieli_wl82_boot_probe(uint32_t stage) {
  if (stage == 105u && !paused_initialized) {
    paused_initialized = h2_atomic_bool_init(&paused, false) == H2_ATOMIC_OK;
...
void h2_jieli_upgrade_write_observer(
  ...
  if (paused_initialized) h2_atomic_bool_store(&paused, true, H2_ATOMIC_RELEASE);
  wdt_close();
  for (;;) {

...
static pref_record_t *find_record(test_fixture_t *fixture, const char *key) {
    for (size_t i = 0u; i < sizeof(fixture->records) / sizeof(fixture->records[0]); ++i) {
        if (strcmp(fixture->records[i].key, key) == 0) return &fixture->records[i];
    }
    return NULL;
}
...
static int pref_get_blob(
    ...
    if (record == NULL || !record->present) return H2_PAL_ERR_NOT_FOUND;
...
static int pref_remove(h2_pal_pref_namespace_t *ns, const char *key) {
    test_fixture_t *fixture = ns->user;
    pref_record_t *record = find_record(fixture, key);
    if (record == NULL) return H2_PAL_ERR_NOT_FOUND;
    fixture->pending_key = key;
...
static int pref_commit(h2_pal_pref_namespace_t *ns) {
    ...
    if (fixture->boot_intent_present == 2) fixture->boot_intent_present = 1;
    if (fixture->last_result_present == 2) fixture->last_result_present = 1;
    return H2_PAL_OK;
}
...
static int fs_remove(void *user, const char *path) {
    ...
    if (!fixture->package_present) return H2_PAL_FS_ERR_NOT_FOUND;
    fixture->package_present = 0;
    return H2_PAL_FS_OK;
}

static int power_running(
    void *user,
...
static int power_set_next(void *user, uint32_t partition_id) {
    ...
    if (fixture->set_next_result != H2_PAL_OK) return fixture->set_next_result;
...
static int image_capacity(void *user, uint32_t partition_id, uint64_t *out) {
    (void)user;
    if (partition_id != 1u && partition_id != 2u) return H2_PAL_ERR_NOT_FOUND;
    *out = 128u;
    return H2_PAL_OK;
}
...
static int image_write(void *user, const void *data, size_t len) {
    ...
    memcpy(fixture->partition_bytes[fixture->writer_partition - 1u] +
        fixture->writer_offset, data, len);
...
static h2_loader_image_identity_t identity(
...
static h2_loader_metadata_t metadata(
    h2_loader_image_role_t role,
    const char *sha) {
...
static h2_loader_metadata_t read_metadata(
    test_fixture_t *fixture,
    h2_loader_metadata_slot_t slot,
    int *present) {
    h2_loader_metadata_t value;
    assert(h2_loader_metadata_read(
               &fixture->pref, &fixture->mem, slot, &value, present) == H2_PAL_OK);
    return value;
}
...
static void test_app_finalize_only_consumes_matching_stage(void) {
    ...
    stage.package_size = 1024u;
    (void)snprintf(stage.package_checksum, sizeof(stage.package_checksum), "%s", SHA_A);
    write_metadata(&fixture, H2_LOADER_METADATA_SLOT_STAGE, &stage);
    assert(h2_loader_finalize_active_app(
               &fixture.pref, &fixture.mem, &fixture.fs,
               H2_LOADER_DEFAULT_PACKAGE_PATH, &app, 2u, 2u) == H2_PAL_OK);
...
static void test_reboot_commands_only_set_intent_and_partition(void) {
    ...
    assert(h2_loader_reboot_app_with_transition(
               &fixture.loader, NULL, NULL) == H2_PAL_OK);
    ...
    fixture.reboot_calls = 0u;
    assert(h2_loader_reboot_upgrade_with_transition(
               &fixture.loader, NULL, NULL) == H2_PAL_OK);
    assert(fixture.boot_intent == H2_LOADER_BOOT_INTENT_AUTO);
    assert(fixture.next_partition == 1u);
    assert(fixture.reboot_calls == 1u);

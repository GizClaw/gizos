...
static void atomic_update_flags(
...
#if defined(_MSC_VER)
        int observed = (int)_InterlockedCompareExchange(
            (volatile long *)value, (long)next, (long)expected);
        if (observed == expected) return;
        expected = observed;
#else
        if (__atomic_compare_exchange_n(
                value, &expected, (int)next, 0,
    ...
    return loader != NULL &&
        (atomic_load(&loader->mfg_gate_bypass) != 0 ||
         ...
         h2_loader_mfg_summary_is_passed(
             &loader->status.mfg, loader->config.mfg_required_total));
}
...
uint32_t h2_loader_get_command_availability(
    ...
    uint32_t available;
    if (loader == NULL || status == NULL) return 0u;
...
static int read_control_fields(
    const h2_pal_pref_api_t *pref,
    ...
    if (ns == NULL) {
        ...
        if (ns->get_u32 != NULL) {
            ...
            if (get_rc != H2_PAL_OK && get_rc != H2_PAL_ERR_NOT_FOUND) rc = get_rc;
        }
...
#undef READ_SLOT
    ...
    if (rc != H2_PAL_OK) return rc;
    if (!present) memset(&out_status->mfg, 0, sizeof(out_status->mfg));
    return H2_PAL_OK;
}

static void status_from_config(
    ...
    (void)h2_loader_status_set_device(
        status, config->board, config->target, config->chip);
    if (role != H2_LOADER_ACTIVE_ROLE_UNKNOWN) {
        (void)h2_loader_status_set_active(
            status, role,
            role == H2_LOADER_ACTIVE_ROLE_APP ? "app" : "loader",
...
int h2_loader_read_status(
    h2_loader_t *loader,
    h2_loader_status_t *out_status) {
    int rc;
    if (loader == NULL || out_status == NULL) return H2_PAL_ERR_INVALID_ARG;
    rc = h2_loader_read_pref_status(
        loader->config.pref, loader->config.package.allocator, out_status);
    if (rc != H2_PAL_OK) return rc;
...
static int identity_valid(const h2_loader_image_identity_t *identity) {
    ...
    return h2_loader_metadata_validate(
        H2_LOADER_METADATA_SLOT_PARTITION_1, &metadata) == H2_PAL_OK;
}

static void metadata_from_identity(
...
static int seed_running_metadata(h2_loader_t *loader) {
    h2_loader_metadata_slot_t slot = slot_for_partition(
        loader, loader->status.running_partition_id);
    ...
    if (stored->valid &&
        stored->image_size == active.image_size &&
    ...
    if (rc == H2_PAL_OK) *stored = active;
    return rc;
}

static void emit_event(
    h2_loader_t *loader,
    h2_loader_startup_event_t event,
    int code) {
...
int h2_loader_set_last_result(h2_loader_t *loader, int result) {
    ...
    if (rc == H2_PAL_OK) loader->status.last_result = result;
    return rc;
}
...
static int remove_optional_file(
    ...
    const char *path) {
    if (path == NULL || path[0] == '\0') return H2_PAL_OK;
...
static int stage_matches_inspection(
    ...
    return stage->valid && !inspection->legacy &&
        ...
        strcmp(stage->image_checksum, inspection->manifest.image_sha256) == 0 &&
...
static int inspect_current_stage(
    ...
    if (rc != H2_PAL_OK) {
        ...
        (void)h2_loader_metadata_write(
            loader->config.pref, H2_LOADER_METADATA_SLOT_STAGE, &invalid);
...
static int finish_stage(
    ...
    h2_loader_metadata_t *stage) {
    h2_loader_metadata_t invalid = {0};
    int rc = h2_loader_metadata_write(
    ...
    if (rc == H2_PAL_OK && stage != NULL) *stage = invalid;
    return rc;
}
...
static int write_partition_2(
    ...
    int rc = h2_loader_metadata_write(loader->config.pref,
    ...
    rc = h2_loader_package_install_to(
        &loader->package, inspection, loader->config.app_partition_id,
        &plan, &result);
    if (rc != H2_PAL_OK) return rc;
    rc = h2_loader_metadata_from_stage(&loader->status.stage, &installed);
    if (rc != H2_PAL_OK) return rc;
    rc = h2_loader_metadata_write(loader->config.pref,
        H2_LOADER_METADATA_SLOT_PARTITION_2, &installed);
    if (rc == H2_PAL_OK) loader->status.partition_2 = installed;
    return rc;
}

static int prepare_disruptive(
    h2_loader_t *loader,
    h2_loader_disruptive_action_t action) {
    return loader->config.before_disruptive != NULL
        ? loader->config.before_disruptive(loader->config.disruptive_user, action)
        : H2_PAL_OK;
}

static int set_next_and_reboot(
    h2_loader_t *loader,
    uint32_t partition_id,
...
static int mount_file_points(h2_loader_t *loader) {
    int rc;
    if (loader->config.mount_file_point == NULL) return H2_PAL_OK;
...
int h2_loader_init(h2_loader_t *loader, const h2_loader_config_t *config) {
    ...
    loader->config.package.data_root = default_if_empty(
        config->package.data_root, H2_LOADER_DEFAULT_DATA_ROOT);
    loader->config.package.installed_checksum_path = default_if_empty(
        config->package.installed_checksum_path,
        H2_LOADER_DEFAULT_CHECKSUM_PATH);
...
int h2_loader_startup(
    h2_loader_t *loader,
    h2_loader_startup_action_t *out_action) {
    h2_loader_package_inspection_t inspection;
    int rc;
    if (loader == NULL || out_action == NULL) return H2_PAL_ERR_INVALID_ARG;
    *out_action = H2_LOADER_STARTUP_ACTION_COMMAND_MODE;
    ...
    if (loader->status.stage.valid) {
        ...
        if (rc != H2_PAL_OK) return fail_recovery(loader, rc);
        if (!h2_loader_metadata_image_equal(
                &loader->status.stage, &loader->status.partition_2)) {
    ...
    if (!mfg_gate_satisfied(loader)) return H2_PAL_OK;
    emit_event(loader, H2_LOADER_STARTUP_EVENT_BOOT_PARTITION_2, H2_PAL_OK);
    rc = set_next_and_reboot(
        loader, loader->config.app_partition_id,
        H2_LOADER_BOOT_INTENT_AUTO,
...
int h2_loader_reboot_h2loader_with_transition(
    ...
    return set_next_and_reboot(loader, loader->config.h2loader_partition_id,
        ...
        H2_LOADER_DISRUPTIVE_REBOOT_LOADER,
        transition, transition_user);
}
...
int h2_loader_reboot_app_with_transition(
    ...
    void *transition_user) {
    if (loader == NULL) return H2_PAL_ERR_INVALID_ARG;
    return set_next_and_reboot(loader, loader->config.app_partition_id,
        H2_LOADER_BOOT_INTENT_AUTO,
        H2_LOADER_DISRUPTIVE_REBOOT_APP,
        transition, transition_user);
}
...
int h2_loader_reboot_upgrade_with_transition(
    ...
    return set_next_and_reboot(loader, loader->config.h2loader_partition_id,
        H2_LOADER_BOOT_INTENT_AUTO,
        H2_LOADER_DISRUPTIVE_REBOOT_UPGRADE,
        transition, transition_user);
}

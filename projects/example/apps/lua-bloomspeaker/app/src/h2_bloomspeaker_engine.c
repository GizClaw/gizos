...
static void make_handshake(uint8_t out[H2_BLOOMSPEAKER_HANDSHAKE_SIZE],
...
static bool handshake_valid(const uint8_t *data, size_t size,
                            uint64_t source, uint64_t target) {
  return size == H2_BLOOMSPEAKER_HANDSHAKE_SIZE &&
         memcmp(data, s_handshake_magic, sizeof(s_handshake_magic)) == 0 &&
         read_le(data + 4u, 5u) == source && read_le(data + 9u, 5u) == target;
}
...
static int accept_claimed_inbound(h2_bloomspeaker_engine_t *engine,
  ...
  engine->pairing_passkey = derive_passkey(
      engine->pairing.local.device_tag, engine->pairing.local.ticket, peer,
      peer_ticket);
...
static int server_handler(void *user, h2_bleikcp_t *stream,
  ...
  if (result == H2_PAL_OK &&
      !handshake_valid(frame, frame_size, engine->peer_tag,
                       engine->pairing.local.device_tag)) {
    result = H2_PAL_ERR_FORMAT;
  }
  if (result == H2_PAL_OK) {
    make_handshake(frame, engine->pairing.local.device_tag, engine->peer_tag,

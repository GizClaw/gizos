...
#include "flappybird_script_generated.h"
...
h2_pal_result_t
h2_lua_flappybird_run(h2_runtime_t *runtime,
  ...
  const h2_lua_resource_t resources[] = {
      {
          .name = "@flappybird/main.lua",
          .source = flappybird_script,
          .source_size = flappybird_script_size,
      },
  };
  char button_id[16];
  const h2_lua_arg_t args[] = {
      {.name = "button_component_id", .value = button_id},
  };
  if (runtime == NULL || config == NULL || config->should_stop == NULL) {
    return H2_PAL_ERR_INVALID_ARG;
  }
  (void)snprintf(button_id, sizeof(button_id), "%u",
                 (unsigned)config->button_component_id);
  ...
  if (job_id != H2_LUA_JOB_ID_NONE &&
    ...
    if (terminal(status.state)) {
      ...
      (void)h2_pal_log_write(runtime->log, H2_PAL_LOG_INFO, "lua-flappybird",
                             diagnostic);
    }

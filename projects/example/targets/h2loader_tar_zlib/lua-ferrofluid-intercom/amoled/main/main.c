...
static h2_pal_result_t confirm_ready(void *user) {
  ...
  if (result == H2_PAL_OK) {
    printf("H2_LUA_FERROFLUID_INTERCOM_READY display=368x448 "
...
static void image_entry(void *user) {
  ...
  (void)user;
  result = h2_lua_ferrofluid_intercom_amoled_runtime_config(&config);
  if (result == H2_PAL_OK) {
    result = h2_esp_h2loader_app_commands_prepare_serial(
        &config, "lua-ferrofluid-intercom", 1u, 3u);
  }
  if (result == H2_PAL_OK)
    result = h2_runtime_init(&config, &runtime);
  if (result == H2_PAL_OK) {
    result = h2_esp_h2loader_app_commands_start(
        runtime, "lua-ferrofluid-intercom", 1u, 3u);
  }
  if (result == H2_PAL_OK) {
    h2_runtime_input_poll_config_t input_poll = {0};
    result = h2_lua_ferrofluid_intercom_amoled_input_poll_config(&input_poll);
    if (result == H2_PAL_OK)
      result = h2_runtime_input_start(runtime, &input_poll);
  }
  if (result != H2_PAL_OK) {
    printf("H2_LUA_FERROFLUID_INTERCOM_FAIL stage=start rc=%d\n",
           (int)result);
    esp_restart();
  }
  result = h2_lua_ferrofluid_intercom_run(
      runtime, &(h2_lua_ferrofluid_intercom_config_t){
                   .back_component_id = H2_RUNTIME_COMPONENT_ID_NONE,
                   .should_stop = never_stop,
                   .on_ready = confirm_ready,
                   .on_ready_user = runtime,
               });
  printf("H2_LUA_FERROFLUID_INTERCOM_EXIT rc=%d recovery=app_command\n",
         (int)result);
  hold_for_recovery();
}
...
void app_main(void) {
  h2_pal_result_t result = h2_esp_board_start_entry_task(
      "amoled/lua-ferrofluid-intercom", image_entry, NULL);
  if (result != H2_PAL_OK) {
    printf("H2_BOARD_ENTRY_FAIL board=amoled "
           "image=lua-ferrofluid-intercom code=%d\n",
           (int)result);
  }
}

...
h2_pal_result_t h2_esp_target_task_policy_install(void) {
  ...
  if (rc != H2_PAL_OK) {
    printf(
        "H2_PAL_TASK_POLICY_FAIL unit=esp "
        "target=projects/example/targets/h2loader_tar_zlib/partial-update/szp "
  ...
  if (rc == H2_PAL_OK) {
    printf("H2_PAL_TASK_POLICY_READY unit=esp "
           "target=projects/example/targets/h2loader_tar_zlib/partial-update/"
           "szp\n");
  } else {
    printf(
        "H2_PAL_TASK_POLICY_FAIL unit=esp "

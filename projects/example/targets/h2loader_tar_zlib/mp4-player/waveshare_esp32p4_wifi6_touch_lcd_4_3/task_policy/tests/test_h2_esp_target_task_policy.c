...
int main(void) {
  ...
  assert(get_policy("net/modem_ppp_rx", &policy) == H2_PAL_ERR_NOT_FOUND);
  assert(get_policy("modem/call_in", &policy) == H2_PAL_ERR_NOT_FOUND);
  assert(get_policy("modem/gnss_fix", &policy) == H2_PAL_ERR_NOT_FOUND);

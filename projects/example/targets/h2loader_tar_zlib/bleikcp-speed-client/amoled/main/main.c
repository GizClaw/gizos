...
static void image_entry(void *user) {
    ...
    printf("H2_BLEIKCP_SPEED_FAIL board=amoled role=client stage=run rc=%d\n", rc);
}
...
void app_main(void) {
    ...
    int rc = h2_esp_board_start_entry_task(
        "amoled/bleikcp-speed-client", image_entry, NULL);
    if (rc != H2_PAL_OK) {
        printf("H2_BOARD_ENTRY_FAIL board=amoled image=bleikcp-speed-client code=%d\n", rc);
    }
}

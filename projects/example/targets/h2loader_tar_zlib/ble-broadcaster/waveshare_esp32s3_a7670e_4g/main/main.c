...
static int smoke_confirm_app(h2_runtime_t *runtime) {
    printf("H2_ESP_SMOKE_BLE_CONFIRM stage=begin\n");
    fflush(stdout);
    int rc = h2_esp_h2loader_app_confirm(runtime);
    printf("H2_ESP_SMOKE_BLE_CONFIRM stage=done rc=%d\n", rc);
    fflush(stdout);
    return rc;
}
...
static void image_entry(void *user) {
    ...
    if (rc == H2_PAL_OK) {
        int diagnostic_rc = h2_smoke_ble_advertising_run(runtime);
        printf("H2_BLE_BROADCASTER_DIAGNOSTIC rc=%d\n", diagnostic_rc);
...
void app_main(void) {
    ...
    h2_pal_result_t rc = h2_esp_board_start_entry_task(
        "waveshare-a7670e/ble-adv", image_entry, NULL);
    if (rc != H2_PAL_OK) {
        printf(
            "H2_BOARD_ENTRY_FAIL board=waveshare_esp32s3_a7670e_4g image=ble-broadcaster code=%d\n",
            rc);
    }
}

...
void app_main(void) {
    h2_pal_result_t rc = h2_esp_board_start_entry_task(
        "waveshare_esp32p4_wifi6_touch_lcd_4_3/crash", image_entry, NULL);
    if (rc != H2_PAL_OK) {
        printf(
            "H2_BOARD_ENTRY_FAIL board=waveshare_esp32p4_wifi6_touch_lcd_4_3 image=crash-before-confirm code=%d\n",
            rc);
    }
}

...
static void crash_now(void *user) {
    (void)user;
    fflush(stdout);
    esp_system_abort("h2loader crash-before-confirm diagnostic");
}
...
void app_main(void) {
    h2_pal_result_t rc = h2_esp_board_start_entry_task(
        "waveshare-a7670e/crash", image_entry, NULL);
    if (rc != H2_PAL_OK) {
        printf(
            "H2_BOARD_ENTRY_FAIL board=waveshare_esp32s3_a7670e_4g image=crash-before-confirm code=%d\n",
            rc);
    }
}

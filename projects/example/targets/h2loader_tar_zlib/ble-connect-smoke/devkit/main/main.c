...
static const uint8_t s_jieli_service_uuid[16] = {
    0x1d, 0x72, 0xa1, 0x6b, 0x3a, 0xaf, 0x0b, 0xaa,
    0xe2, 0x53, 0xd8, 0x3e, 0x70, 0xb5, 0xa4, 0x71,
};
static const uint8_t s_jieli_vendor_service_uuid[2] = {0x30, 0xaf};

typedef struct scan_state {
    h2_atomic_bool_t found;
...
static bool on_scan_result(void *user, const h2_pal_ble_scan_result_t *result) {
    ...
    if (h2_atomic_load_explicit(&state->found, H2_ATOMIC_ACQUIRE)) return true;
    bool matches = false;
    for (size_t i = 0; i < result->service_uuid_count; ++i) {
        if ((result->service_uuids[i].len == sizeof(s_jieli_service_uuid) &&
             memcmp(result->service_uuids[i].data, s_jieli_service_uuid,
                    sizeof(s_jieli_service_uuid)) == 0) ||
            (result->service_uuids[i].len == sizeof(s_jieli_vendor_service_uuid) &&
             memcmp(result->service_uuids[i].data, s_jieli_vendor_service_uuid,
                    sizeof(s_jieli_vendor_service_uuid)) == 0)) {
            matches = true;
            break;
        }
    }
    ...
    h2_atomic_store_explicit(&state->found, true, H2_ATOMIC_RELEASE);
    printf("H2_DEVKIT_JIELI stage=found rssi=%d type=%u addr_type=%u\n",
...
static void image_entry(void *user) {
    ...
    printf("H2_DEVKIT_JIELI stage=runtime rc=%d\n", rc);
    if (rc == H2_PAL_OK) {
        rc = h2_pal_ble_start(runtime->ble_host);
    }
    printf("H2_DEVKIT_JIELI stage=ready rc=%d\n", rc);

    for (unsigned attempt = 1u; rc == H2_PAL_OK; ++attempt) {
        scan_state_t scan = {0};
        if (h2_atomic_init(&scan.found, false) != H2_ATOMIC_OK) {
        ...
        printf("H2_DEVKIT_JIELI stage=scan attempt=%u\n", attempt);
        rc = h2_pal_ble_start_scan(
            runtime->ble_host, &params, on_scan_result, &scan);
        for (unsigned elapsed = 0u; rc == H2_PAL_OK && !h2_atomic_load_explicit(&scan.found, H2_ATOMIC_ACQUIRE) &&
             elapsed < 5500u; elapsed += 50u) {
            vTaskDelay(pdMS_TO_TICKS(50u));
        }
        (void)h2_pal_ble_stop_scan(runtime->ble_host);
        if (rc != H2_PAL_OK || !h2_atomic_load_explicit(&scan.found, H2_ATOMIC_ACQUIRE)) {
            printf("H2_DEVKIT_JIELI stage=scan-result attempt=%u found=0 rc=%d\n",
                   attempt, rc);
            rc = H2_PAL_OK;
            h2_atomic_destroy(&scan.found);
        ...
        printf("H2_DEVKIT_JIELI stage=connect attempt=%u rssi=%d\n",
               attempt, peer_rssi);
        int connect_rc = h2_pal_ble_connect(
            runtime->ble_host, &peer_addr, &connect_params, &conn_handle);
        printf("H2_DEVKIT_JIELI stage=connect-result attempt=%u rc=%d handle=%u\n",
        ...
        if (connect_rc == H2_PAL_OK) {
            ...
            printf("H2_DEVKIT_JIELI stage=disconnect rc=%d\n", disconnect_rc);
        }
        vTaskDelay(pdMS_TO_TICKS(1000u));
    }
    printf("H2_DEVKIT_JIELI stage=fatal rc=%d\n", rc);
    for (;;) {
        vTaskDelay(pdMS_TO_TICKS(1000u));
    }
}
...
void app_main(void) {
    ...
    int rc = h2_esp_board_start_entry_task(
        "devkit/ble-connect-smoke", image_entry, NULL);
    if (rc != H2_PAL_OK) {
        printf("H2_BOARD_ENTRY_FAIL board=devkit image=ble-connect-smoke code=%d\n",
               rc);
    }
}

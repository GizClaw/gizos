...
static h2_pal_result_t mapper_list(void *user, h2_runtime_component_t filter,
  ...
  entry = (h2_runtime_component_mapping_entry_t){
      .component_id = H2_LUA_QI_DUEL_COMPONENT_BACK,
...
static h2_pal_result_t mapper_get(void *user,
  ...
  if (out_periph_id == NULL)
    return H2_PAL_ERR_INVALID_ARG;
  if (component_id != H2_LUA_QI_DUEL_COMPONENT_BACK)
...
h2_pal_result_t
h2_lua_qi_duel_amoled_runtime_config(h2_runtime_config_t *out_config) {
  esp_rom_printf("H2_LUA_QI_DUEL_BOOT stage=task_policy_begin\n");
  h2_pal_result_t result = h2_esp_target_task_policy_install();
  esp_rom_printf("H2_LUA_QI_DUEL_BOOT stage=task_policy_end rc=%d\n",
                 (int)result);
  if (result != H2_PAL_OK)
    return result;
  esp_rom_printf("H2_LUA_QI_DUEL_BOOT stage=board_runtime_begin\n");
  result = h2_esp_board_runtime_config(out_config);
  esp_rom_printf("H2_LUA_QI_DUEL_BOOT stage=board_runtime_end rc=%d\n",
                 (int)result);
  if (result != H2_PAL_OK)
    return result;
...
h2_pal_result_t h2_lua_qi_duel_amoled_input_poll_config(

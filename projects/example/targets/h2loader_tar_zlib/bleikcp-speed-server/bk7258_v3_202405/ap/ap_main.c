...
static int advertise_ble_service(
    ...
    (void)user;
    return h2_bk_h2loader_advertise_app_ble_service(service_uuid);
}
...
static void app_entry(void *user) {
    ...
    if (rc == H2_PAL_OK) {
        rc = h2_bk_h2loader_start_app_ble_extended(
            runtime, "bleikcp-speed-server");
    ...
    emit_marker("H2_BLEIKCP_SPEED_FAIL board=bk7258_v3_202405 role=server rc=%d", rc);
}
...
int main(void) {
    ...
    int rc = h2_bk7258_board_start_entry_task(
        "bk/bleikcp-speed-server", app_entry, NULL);
    if (rc != H2_PAL_OK) {
        os_printf("H2_BK_BOARD_ENTRY_FAIL image=bleikcp-speed-server rc=%d\r\n", rc);
    }
    return 0;
}

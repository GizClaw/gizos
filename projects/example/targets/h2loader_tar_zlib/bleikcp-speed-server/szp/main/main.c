...
static void image_entry(void *user) {
    ...
    printf("H2_BLEIKCP_SPEED_FAIL board=szp role=server stage=run rc=%d\n", rc);
}
...
void app_main(void) {
    ...
    int rc = h2_esp_board_start_entry_task(
        "szp/bleikcp-speed-server", image_entry, NULL);
    if (rc != H2_PAL_OK) {
        printf("H2_BOARD_ENTRY_FAIL board=szp image=bleikcp-speed-server code=%d\n", rc);
    }
}

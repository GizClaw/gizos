...
static const h2_trie_route_t s_routes[] = {
    ...
    {H2_LUA_WORKER_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_5_policy},
    {H2_LUA_COSMIC_DRIFT_ENTRY_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT,
     handle_policy, &s_priority_4_policy},
};
...
enum {
  ROUTE_NODE_CAPACITY =
      ...
      H2_TRIE_LITERAL_NODE_COUNT(H2_RUNTIME_INPUT_TASK_NAME_VALUE) +
      H2_TRIE_LITERAL_NODE_COUNT(H2_LUA_WORKER_TASK_NAME_VALUE) +
      H2_TRIE_LITERAL_NODE_COUNT(H2_LUA_COSMIC_DRIFT_ENTRY_TASK_NAME_VALUE),
};
...
h2_pal_result_t h2_esp_target_task_policy_install(void) {
  ...
  if (result != H2_PAL_OK) {
    printf("H2_PAL_TASK_POLICY_FAIL unit=esp "
           "target=projects/example/targets/h2loader_tar_zlib/"
           "lua-cosmic-drift/amoled stage=routes reason=%d\n",
           result);
    return result;
  }
  ...
  if (result == H2_PAL_OK) {
    printf("H2_PAL_TASK_POLICY_READY unit=esp "
           "target=projects/example/targets/h2loader_tar_zlib/"
           "lua-cosmic-drift/amoled\n");
  } else {
    printf("H2_PAL_TASK_POLICY_FAIL unit=esp "
           "target=projects/example/targets/h2loader_tar_zlib/"
           "lua-cosmic-drift/amoled stage=configure reason=%d\n",
           result);
  }
  return result;
}

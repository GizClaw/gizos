...
static h2_pal_result_t confirm_ready(void *user) {
  ...
  if (result == H2_PAL_OK) {
    printf(
        "H2_LUA_COSMIC_DRIFT_READY display=368x448 touch=ft3168 button=boot\n");
  }
  return result;
}
...
static void image_entry(void *user) {
  ...
  (void)user;
  result = h2_lua_cosmic_drift_amoled_runtime_config(&config);
  if (result == H2_PAL_OK) {
    result = h2_esp_h2loader_app_commands_prepare_serial(
        &config, "lua-cosmic-drift", 1u, 3u);
  }
  if (result == H2_PAL_OK)
    result = h2_runtime_init(&config, &runtime);
  if (result == H2_PAL_OK) {
    result =
        h2_esp_h2loader_app_commands_start(runtime, "lua-cosmic-drift", 1u, 3u);
  }
  if (result == H2_PAL_OK) {
    h2_runtime_input_poll_config_t input_poll = {0};
    result = h2_lua_cosmic_drift_amoled_input_poll_config(&input_poll);
    if (result == H2_PAL_OK)
      result = h2_runtime_input_start(runtime, &input_poll);
  }
  if (result != H2_PAL_OK) {
    printf("H2_LUA_COSMIC_DRIFT_FAIL stage=start rc=%d\n", (int)result);
    esp_restart();
  }
  result = h2_lua_cosmic_drift_run(
  ...
  printf("H2_LUA_COSMIC_DRIFT_EXIT rc=%d recovery=app_command\n", (int)result);
  hold_for_recovery();
}

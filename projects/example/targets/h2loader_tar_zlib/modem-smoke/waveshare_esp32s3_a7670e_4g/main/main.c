...
static void image_entry(void *user) {
    ...
    if (rc == H2_PAL_OK) {
        rc = h2_esp_h2loader_app_commands_prepare_serial(
            &runtime_config, "modem-smoke", 1u, 3u);
    }
    if (rc == H2_PAL_OK) {
    ...
    if (rc == H2_PAL_OK) {
        rc = h2_esp_h2loader_app_commands_start(
            runtime, "modem-smoke", 1u, 3u);
    }
    if (rc == H2_PAL_OK) {
        rc = smoke_confirm_app(runtime);
    }
    if (rc == H2_PAL_OK) {
        rc = h2_modem_smoke_run(runtime, NULL);
    }
    printf("H2_ESP_SMOKE_MODEM_SMOKE_DONE rc=%d\n", rc);
    fflush(stdout);
    for (;;) {
...
void app_main(void) {
    ...
    h2_pal_result_t rc = h2_esp_board_start_entry_task(
        "waveshare-a7670e/modem-smoke", image_entry, NULL);
    if (rc != H2_PAL_OK) {
        printf(
            "H2_BOARD_ENTRY_FAIL board=waveshare_esp32s3_a7670e_4g image=modem-smoke code=%d\n",
            rc);
    }
}

...
static h2_pal_result_t mapper_list(void *user, h2_runtime_component_t filter,
  ...
  entry = (h2_runtime_component_mapping_entry_t){
      .component_id = H2_LUA_FISHING_GAME_COMPONENT_BACK,
...
static h2_pal_result_t mapper_get(void *user,
  ...
  if (out_periph_id == NULL)
    return H2_PAL_ERR_INVALID_ARG;
  if (component_id != H2_LUA_FISHING_GAME_COMPONENT_BACK)
...
h2_pal_result_t
h2_lua_fishing_game_amoled_runtime_config(h2_runtime_config_t *out_config) {
...
h2_pal_result_t h2_lua_fishing_game_amoled_input_poll_config(

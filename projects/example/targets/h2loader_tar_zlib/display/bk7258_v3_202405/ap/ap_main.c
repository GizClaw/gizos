...
static void app_entry(void *user) {
    ...
    emergency_uart_write_string(0, "H2_BK_AP_BOOT image=display\r\n");
    os_printf("H2_BK_AP_BOOT image=display\r\n");
    rtos_delay_milliseconds(500);
    emit_marker("H2_BK_SMOKE_DISPLAY_STAGE stage=host_cli_owner_cp");
    rc = h2_bk7258_board_runtime_config(&runtime_config);
    if (rc != H2_PAL_OK) {
        emit_marker("H2_BK_SMOKE_DISPLAY_FAIL stage=runtime_config rc=%d", rc);
    ...
    if (rc != H2_PAL_OK) {
        emit_marker("H2_BK_SMOKE_DISPLAY_FAIL stage=runtime_init rc=%d", rc);
        (void)h2_bk_h2loader_reboot_to_loader();
        for (;;) {
            rtos_delay_milliseconds(1000);
        }
    ...
    if (rc != H2_PAL_OK) {
        emit_marker("H2_BK_SMOKE_DISPLAY_FAIL stage=app_cli rc=%d", rc);
        (void)h2_bk_h2loader_reboot_to_loader();
        for (;;) {
            rtos_delay_milliseconds(1000);
        }
    ...
    if (rc != H2_DISPLAY_OK) {
        emit_marker("H2_BK_SMOKE_DISPLAY_FAIL stage=run rc=%d", rc);
    } else {
        emit_marker("H2_BK_SMOKE_DISPLAY_STAGE stage=run_done");
        rtos_delay_milliseconds(500);
        emit_marker("H2_BK_SMOKE_DISPLAY_STAGE stage=confirm_begin");
        rc = h2_bk_h2loader_confirm_current_app(runtime->pref);
        if (rc != H2_PAL_OK) {
            emit_marker("H2_BK_SMOKE_DISPLAY_FAIL stage=confirm rc=%d", rc);
        } else {
            emit_marker("H2_BK_SMOKE_DISPLAY_READY rc=0");
        }
    }
    for (;;) {
        rtos_delay_milliseconds(1000);
...
int main(void) {
    ...
    emergency_uart_write_string(0, "H2_BK_AP_MAIN image=display stage=before_bk_init\r\n");
    bk_init();
    emergency_uart_write_string(0, "H2_BK_AP_MAIN image=display stage=after_bk_init\r\n");
    h2_pal_result_t rc = h2_bk7258_board_start_entry_task(
        "bk/display", app_entry, NULL);
    if (rc != H2_PAL_OK) {
        emit_marker("H2_BK_BOARD_ENTRY_FAIL image=display rc=%d", rc);
    }
    return 0;
}

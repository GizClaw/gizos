...
static const h2_trie_route_t s_routes[] = {
    ...
    {H2_LOADER_BLE_LINK_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_6_policy},
    /*
     * The provisioning worker blocks in Wi-Fi scan and connect calls. It sits
     * below the loader command tasks so a stuck scan cannot starve them.
     */
    {H2_BLE_WIFI_CONFIG_WORKER_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT,
     handle_policy, &s_priority_5_policy},
};
...
enum {
  ROUTE_NODE_CAPACITY = 1u + H2_TRIE_LITERAL_NODE_COUNT(H2LOADER_APP_COMMAND_TASK_NAME_VALUE) +
                        ...
                        H2_TRIE_LITERAL_NODE_COUNT(
                            H2_BLE_WIFI_CONFIG_WORKER_TASK_NAME_VALUE),
};

...
static const h2_trie_route_t s_routes[] = {
    ...
    {H2_LOADER_BLE_LINK_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_6_policy},
    {H2_BLOOMSPEAKER_BLE_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_6_policy},
    {H2_BLOOMSPEAKER_AUDIO_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_7_policy},
    {H2_BLEIKCP_WORKER_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_7_policy},
    {H2_BLEIKCP_SERVER_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_5_policy},
    {H2_PEER_NETWORK_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_7_policy},
    {H2_PEER_UDP_TASK_NAME_VALUE, H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_7_policy},
};

enum {
  ROUTE_NODE_CAPACITY =
      1u + H2_TRIE_LITERAL_NODE_COUNT(H2LOADER_APP_COMMAND_TASK_NAME_VALUE) +
      H2_TRIE_LITERAL_NODE_COUNT(H2_LOADER_RETURN_TASK_NAME_VALUE) +
      H2_TRIE_LITERAL_NODE_COUNT(H2_LOADER_BLE_LINK_TASK_NAME_VALUE) +
      H2_TRIE_LITERAL_NODE_COUNT(H2_BLOOMSPEAKER_BLE_TASK_NAME_VALUE) +
      H2_TRIE_LITERAL_NODE_COUNT(H2_BLOOMSPEAKER_AUDIO_TASK_NAME_VALUE) +
...
h2_pal_result_t h2_esp_target_task_policy_install(void) {
  h2_pal_result_t result = h2_trie_build(
      &s_router, s_route_nodes, ROUTE_NODE_CAPACITY, s_routes,
      sizeof(s_routes) / sizeof(s_routes[0]));
  if (result == H2_PAL_OK)
    result = h2_trie_set_fallback(&s_router, handle_default_policy, NULL);
  if (result != H2_PAL_OK)
    return result;
  ...
  result = h2_esp_platform_task_configure(&config);
  printf("H2_PAL_TASK_POLICY_%s unit=esp "
         "target=bloomspeaker/amoled reason=%d\n",
         result == H2_PAL_OK ? "READY" : "FAIL", result);
  return result;
}

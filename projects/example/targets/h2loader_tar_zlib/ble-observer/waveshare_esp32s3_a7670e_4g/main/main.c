...
static void image_entry(void *user) {
    ...
    if (rc == H2_PAL_OK) {
        int diagnostic_rc = h2_smoke_ble_observer_run(runtime);
        printf("H2_BLE_OBSERVER_DIAGNOSTIC rc=%d\n", diagnostic_rc);
...
void app_main(void) {
    ...
    if (rc != H2_PAL_OK) {
        printf(
            "H2_BOARD_ENTRY_FAIL board=waveshare_esp32s3_a7670e_4g image=ble-observer code=%d\n",
            rc);
    }
}

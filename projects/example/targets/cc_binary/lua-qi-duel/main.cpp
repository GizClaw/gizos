...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (std::strncmp(argv[i], "--layer=", 8) == 0) layer = argv[i] + 8;
      ...
      std::fprintf(stderr, "Default: native top-down game entry. Use --rehearsal for visual-only controls or --game with fixed-time probes.\n");
      std::fprintf(stderr, "Usage: %s [--layer=full|walls|wheel|arena|dust|particles|opponent|hand-left|hand-right|arena-dust|scene7|hud|scene8|carousel-frame|charge-cells|charge-base|scene11|carousel|skill-charge|skill-wave|skill-absorb|skill-guard] [--time-ms=N] [--selected=0..3] [--drag=-90..90] [--health-fx=player-down|player-up|enemy-down|enemy-up] [--charge-fx=down|up] [--impact=combo|armor-break] [--clash=equal|player-combo|enemy-combo|both-combo] [--result=win|lose] [--capture=new.ppm]\n", argv[0]);
      return 2;
    }
  }
  char *end = nullptr;
  ...
  bool valid_clash = clash == nullptr || std::strcmp(clash,"equal")==0 ||
                     std::strcmp(clash,"player-combo")==0 ||
                     std::strcmp(clash,"enemy-combo")==0 ||
                     std::strcmp(clash,"both-combo")==0;
  bool valid_settlement = settlement == nullptr || std::strcmp(settlement,"win")==0 ||
                          std::strcmp(settlement,"lose")==0;
  ...
  if (!valid_layer || !valid_fx || !valid_action || !valid_actor || !valid_impact || !valid_clash || !valid_settlement || !valid_number(qi,0,5,true) || !valid_number(selected,0,3,true) || !valid_number(drag,-90,90,false) ||
  ...
  if (!capture && h2::desktop::open_audio(true, &audio) == H2_PAL_OK) {
  ...
  const h2_lua_qi_duel_config_t config = {
      ...
      .qi = qi,
      .click_controls = 1,
      .battle = game_probe || (!rehearsal && !time_ms && !action && !clash && !settlement && std::strcmp(layer,"full")==0),
      .selected = selected,
      .drag = drag,
      .action = action,
      .actor = actor,

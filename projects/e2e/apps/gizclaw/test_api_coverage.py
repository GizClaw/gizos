...
def passing_log(rules):
    ...
    for case in ("device-api", "connectivity", "rpc", "firmware", "voice", "concurrency", "service", "resource"):
        emit_case(case)
    lines.append(record(stage="summary", **IDENTITY, suite="all", selected=8, terminal=8,
                        **{"pass": 8}, fail=0, error=0, blocked=0, cancelled=0,
...
class CoverageTest(unittest.TestCase):
    ...
    def test_independent_matrix_matches_all_226_approved_functions(self):
        ...
        self.assertEqual(len(self.rules), 226)
    ...
    def test_all_functions_need_ordered_calls_and_assertion(self):
        ...
        self.assertEqual(result["covered"], 226)
    ...
    def test_api_key_state_mutations_require_observed_snapshot(self):
        ...
        for method in ("create", "request_refresh", "request_revoke", "close"):
            ...
            # A successful mutation and claimed assertion without an intervening
            # snapshot must never establish live coverage.
            lines = passing_log([rule])
    ...
    def test_social_ping_and_public_profile_require_real_calls_and_assertions(self):
        ...
        for case, methods in expected.items():
            with self.subTest(case=case):
                symbols = {"h2_gizclaw_" + kind + "_" + method
                           for kind in ("req_create", "resp_parse", "rpc")
                           for method in methods}
                rules = [rule for rule in self.rules if rule.symbol in symbols]
                self.assertEqual({rule.symbol for rule in rules}, symbols)
                self.assertEqual({rule.case for rule in rules}, {case})
                self.assertEqual({rule.assertion_stage for rule in rules},
                                 {method + "-assert" for method in methods})
                lines = passing_log([rule for rule in self.rules if rule not in rules])
                result = self.audit(lines)
                self.assertFalse(result["valid"])
                self.assertEqual(result["missing"], len(symbols))

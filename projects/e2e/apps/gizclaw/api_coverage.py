...
def requirements():
    ...
    # Refresh and close are only proven by the state observed afterwards.
    snapshot = PREFIX + "api_key_state_snapshot"
    for method in "create request_refresh snapshot close destroy".split():
        symbol = PREFIX + "api_key_state_" + method
        observed = method in {"create", "request_refresh", "close"}
...
def validate_inventory(rules, text):
    ...
    if (len(inventory) != 223 or len(set(inventory)) != 223 or
            len(names) != 223 or len(set(names)) != 223 or set(names) != set(inventory)):
        raise ValueError("coverage matrix does not match the approved 223-function inventory")

"""Check real case instrumentation with a boundary double, never live E2E."""
...
class ContactCoverageTest(unittest.TestCase):
    def test_case_output_matches_all_fifteen_contact_rules(self):
        root = api_coverage.repository_root()
        executable = root / "projects/e2e/apps/gizclaw/app/gizclaw_e2e_contact_test"
        if os.name == "nt":
            executable = Path(str(executable) + ".exe")
        run = subprocess.run([str(executable), "--emit-success-evidence"],
                             capture_output=True, text=True, timeout=30, check=False)
        self.assertEqual(run.returncode, 0, "Contact boundary fixture failed")
        ...
        expected = {rule.symbol for rule in rules if rule.case == "rpc/contact"}
        self.assertEqual(len(expected), 15)
        self.assertEqual(observed, expected)
        # No real server, other cases, or Desktop summary: acceptance must fail.
        self.assertFalse(result["valid"])
        self.assertEqual(result["missing"], 221 - 15)

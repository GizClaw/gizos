...
static void fail(const char *stage, int rc, int commands_started) {
  printf("H2_IPERF_E2E_AMOLED stage=%s status=ERROR rc=%d\n", stage, rc);
  fflush(stdout);
  if (!commands_started) {
    esp_restart();
  }
...
static void wait_for_wifi(h2_runtime_t *runtime,
  ...
  for (;;) {
    ...
    if (rc == H2_PAL_OK) {
      ...
      do {
        ...
        if (status_rc == H2_PAL_OK &&
          ...
          printf("H2_IPERF_E2E_AMOLED stage=wifi status=READY "
    ...
    printf("H2_IPERF_E2E_AMOLED stage=wifi status=WAIT rc=%d retry_ms=3000\n",
           rc);
    fflush(stdout);
...
static int apply_power_save(h2_runtime_t *runtime) {
  ...
  printf("H2_IPERF_E2E_AMOLED stage=power_save mode=%d rc=%d\n", (int)mode,
         rc);
  fflush(stdout);
  return rc;
}
...
static void image_entry(void *user) {
  ...
  const h2_iperf_e2e_config_t config = {
      ...
      .port = H2_IPERF_AMOLED_PORT,
      .sctp_udp_port = H2_IPERF_AMOLED_SCTP_UDP_PORT,
...
void app_main(void) {
  ...
  const h2_pal_result_t rc =
      h2_esp_board_start_entry_task("amoled/iperf", image_entry, NULL);
  if (rc != H2_PAL_OK) {

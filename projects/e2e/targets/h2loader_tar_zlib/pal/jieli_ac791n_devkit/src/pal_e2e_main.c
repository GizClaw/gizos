...
static int reboot(void *user, uint32_t reason) {
  (void)user;
  printf("H2_PAL_E2E_REBOOT reason=%u next=%u\r\n", reason, next_partition);
  os_time_dly(10u);
...
static void run_suites(void *user) {
  ...
  printf("H2_PAL_E2E phase=runtime result=%d\r\n", result);
  if (result == H2_PAL_OK) {
    for (size_t s = 0; s < sizeof(suites) / sizeof(suites[0]); ++s) {
      ...
      printf("H2_PAL_E2E phase=suite_begin suite=%u\r\n", suites[s]);
      ...
      for (size_t i = 0; i < report->case_count; ++i) {
        printf("H2_PAL_E2E suite=%u case=%u result=%d\r\n", suites[s],
  ...
  for (;;) {
    ...
    printf("H2_PAL_E2E result=%d passed=%u failed=%u\r\n", result,
...
void app_main(void) {
  ...
  printf("H2_PAL_E2E phase=commands result=%d\r\n", result);
  ...
  printf("H2_PAL_E2E phase=runner result=%d\r\n", result);

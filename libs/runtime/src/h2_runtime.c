...
/* The Time PAL observer preserves the provider's user and all read methods. */
static void runtime_time_adjusted(void *user, uint64_t wall_ms) {
...
#define H2_RUNTIME_BIND_PROXY(field)                                                \
    ...
    private_state->time_proxy.wall_adjusted = runtime_time_adjusted;
    private_state->time_proxy.wall_adjusted_user = runtime;

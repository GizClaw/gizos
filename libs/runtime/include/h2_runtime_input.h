 ...
 * them and counts the drop). Events never precede their snapshot: if readers
 * keep every retired state slot pinned through three short retries, the
 * release events are dropped and counted too. If the input writer mutex itself cannot be
 ...
 * status below and h2_runtime_input_stage_name() are unaffected (with no log
 * referencing the names, an image that does not call it links none of them). The stop then latches `worker_result`,

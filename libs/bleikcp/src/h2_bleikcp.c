...
int h2_bleikcp_resolve_config(
    ...
    if (out->value.worker_task_options.name == NULL) out->value.worker_task_options.name = "bleikcp/kcp";
    if (out->value.worker_task_options.min_stack_size < 6u * 1024u) out->value.worker_task_options.min_stack_size = 6u * 1024u;
    if (out->value.server_task_options.name == NULL) out->value.server_task_options.name = "bleikcp/server";

...
static const uint32_t h2_h2loader_host_sha256_constants[64] = {
...
static void h2_h2loader_host_sha256_transform(
    ...
    for (size_t i = 0u; i < 64u; ++i) {
        ...
        uint32_t temp1 = h + s1 + choice +
            h2_h2loader_host_sha256_constants[i] + schedule[i];
        uint32_t s0 =
...
void h2_h2loader_host_sha256_update(
    ...
    while (len > 0u) {
        ...
        if (sha->block_len == sizeof(sha->block)) {
            h2_h2loader_host_sha256_transform(sha, sha->block);
            sha->block_len = 0u;
        }
    }
}
...
void h2_h2loader_host_sha256_finish(
    ...
    total_bits = sha->total_bytes * UINT64_C(8);
    for (size_t i = 0u; i < 8u; ++i) {
        sha->block[63u - i] = (uint8_t)(total_bits >> (i * 8u));
    }
    h2_h2loader_host_sha256_transform(sha, sha->block);
    for (size_t i = 0u; i < 8u; ++i) {
        write_be32(&out[i * 4u], sha->state[i]);
    }
    memset(sha, 0, sizeof(*sha));
}
...
void h2_h2loader_host_sha256_hex(
    ...
    out[H2_H2LOADER_HOST_SHA256_HEX_LEN] = '\0';
}

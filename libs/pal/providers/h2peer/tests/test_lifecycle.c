...
typedef struct test_webrtc_callbacks {
  void *user;
  void (*on_peer_state)(void *, h2_pal_webrtc_peer_t *,
                        h2_pal_webrtc_peer_state_t);
  void (*on_local_sdp)(void *, h2_pal_webrtc_peer_t *, h2_pal_webrtc_sdp_type_t,
                       h2_pal_webrtc_str_t);
  void (*on_channel_state)(void *, h2_pal_webrtc_peer_t *,
                           h2_pal_webrtc_channel_t *,
                           const h2_pal_webrtc_channel_info_t *,
                           h2_pal_webrtc_channel_state_t);
  void (*on_channel_message)(void *, h2_pal_webrtc_peer_t *,
                             h2_pal_webrtc_channel_t *,
                             const h2_pal_webrtc_channel_info_t *,
                             const uint8_t *, size_t, int);
  void (*on_opus_frame)(void *, h2_pal_webrtc_peer_t *, const uint8_t *,
                        size_t);
} test_webrtc_callbacks_t;

#define h2_pal_webrtc_callbacks_t test_webrtc_callbacks_t

typedef struct test_peer_observer {
  h2_pal_webrtc_peer_t *peer;
  const h2_pal_webrtc_api_t *api;
  test_webrtc_callbacks_t callbacks;
} test_peer_observer_t;

static test_peer_observer_t s_peer_observers[64];
...
test_dtls_set_remote_fingerprint(void *user, void *session,
  ...
  assert(user == session);
  assert(fingerprint.data != NULL && fingerprint.len > sizeof("sha-256 ") - 1u);
...
static test_webrtc_callbacks_t *
test_callbacks_for_peer(h2_pal_webrtc_peer_t *peer) {
  for (size_t i = 0u;
       i < sizeof(s_peer_observers) / sizeof(s_peer_observers[0]); ++i) {
    if (s_peer_observers[i].peer == peer)
      return &s_peer_observers[i].callbacks;
  }
  return NULL;
}

static h2_pal_result_t
test_peer_create_adapter(const h2_pal_webrtc_api_t *api,
    const test_webrtc_callbacks_t *callbacks,
    h2_pal_webrtc_peer_t **out_peer) {
  h2_pal_result_t result = (h2_pal_webrtc_peer_create)(api, out_peer);
  if (result != H2_PAL_OK)
    return result;
  for (size_t i = 0u;
       i < sizeof(s_peer_observers) / sizeof(s_peer_observers[0]); ++i) {
    if (s_peer_observers[i].peer == *out_peer) {
      s_peer_observers[i].api = api;
      s_peer_observers[i].callbacks = *callbacks;
      return H2_PAL_OK;
    }
  }
  for (size_t i = 0u;
       i < sizeof(s_peer_observers) / sizeof(s_peer_observers[0]); ++i) {
    if (s_peer_observers[i].peer == NULL) {
      s_peer_observers[i].peer = *out_peer;
      s_peer_observers[i].api = api;
      s_peer_observers[i].callbacks = *callbacks;
      return H2_PAL_OK;
    }
  }
  (h2_pal_webrtc_peer_close)(api, *out_peer);
  *out_peer = NULL;
  return H2_PAL_ERR_NO_SPACE;
}

static void test_dispatch_event(h2_pal_webrtc_event_t *event) {
  test_webrtc_callbacks_t *callbacks = test_callbacks_for_peer(event->peer);
  if (callbacks == NULL)
    return;
  switch (event->kind) {
  case H2_PAL_WEBRTC_EVENT_PEER_STATE:
    if (callbacks->on_peer_state != NULL)
      callbacks->on_peer_state(callbacks->user, event->peer, event->peer_state);
    break;
  case H2_PAL_WEBRTC_EVENT_LOCAL_SDP:
    if (callbacks->on_local_sdp != NULL)
      callbacks->on_local_sdp(callbacks->user, event->peer, event->sdp_type,
                              event->sdp);
    break;
  case H2_PAL_WEBRTC_EVENT_CHANNEL_STATE:
    if (callbacks->on_channel_state != NULL)
      callbacks->on_channel_state(callbacks->user, event->peer, event->channel,
                                  &event->channel_info, event->channel_state);
    break;
  case H2_PAL_WEBRTC_EVENT_CHANNEL_MESSAGE:
    if (callbacks->on_channel_message != NULL)
      callbacks->on_channel_message(
          callbacks->user, event->peer, event->channel, &event->channel_info,
          event->data, event->data_len, event->is_text);
    break;
  case H2_PAL_WEBRTC_EVENT_OPUS_FRAME:
    if (callbacks->on_opus_frame != NULL)
      callbacks->on_opus_frame(callbacks->user, event->peer, event->data,
                               event->data_len);
    break;
...
static void test_drain_queued_events(const h2_pal_webrtc_api_t *api,
                                     h2_pal_webrtc_peer_t *peer);

static h2_pal_result_t test_peer_poll_adapter(const h2_pal_webrtc_api_t *api,
                                              h2_pal_webrtc_peer_t *peer,
    int timeout_ms) {
  ...
  if (result == H2_PAL_ERR_TIMEOUT)
    return H2_PAL_OK;
  if (result != H2_PAL_OK) {
    test_drain_queued_events(api, peer);
    return result;
  }
  test_dispatch_event(&event);
  int peer_closed = peer->closed;
...
static void test_drain_queued_events(const h2_pal_webrtc_api_t *api,
                                     h2_pal_webrtc_peer_t *peer) {
  while (peer != NULL && peer->direct_event_head != NULL) {
    h2_pal_webrtc_event_t event = {0};
...
static h2_pal_result_t
test_peer_start_offer_adapter(const h2_pal_webrtc_api_t *api,
                              h2_pal_webrtc_peer_t *peer) {
  h2_pal_result_t result = (h2_pal_webrtc_peer_start_offer)(api, peer);
  test_drain_queued_events(api, peer);
  return result;
}

static h2_pal_result_t test_peer_set_remote_sdp_adapter(
    const h2_pal_webrtc_api_t *api, h2_pal_webrtc_peer_t *peer,
    h2_pal_webrtc_sdp_type_t type, h2_pal_webrtc_str_t sdp) {
  h2_pal_result_t result =
      (h2_pal_webrtc_peer_set_remote_sdp)(api, peer, type, sdp);
  test_drain_queued_events(api, peer);
  return result;
}

static h2_pal_result_t test_peer_create_data_channel_adapter(
    const h2_pal_webrtc_api_t *api, h2_pal_webrtc_peer_t *peer,
    const h2_pal_webrtc_channel_config_t *config,
    h2_pal_webrtc_channel_t **out_channel) {
  h2_pal_result_t result =
      (h2_pal_webrtc_peer_create_data_channel)(api, peer, config, out_channel);
  test_drain_queued_events(api, peer);
  return result;
}

static void test_channel_close_adapter(const h2_pal_webrtc_api_t *api,
                                       h2_pal_webrtc_channel_t *channel) {
...
static h2_pal_result_t test_receive_rtp_adapter(h2_pal_webrtc_peer_t *peer,
                                                const uint8_t *packet,
                                                size_t packet_len) {
  h2_pal_result_t result =
      (h2_peer_receive_rtp_for_test)(peer, packet, packet_len);
  for (size_t i = 0u;
       i < sizeof(s_peer_observers) / sizeof(s_peer_observers[0]); ++i) {
    if (s_peer_observers[i].peer == peer) {
      test_drain_queued_events(s_peer_observers[i].api, peer);
      break;
    }
  }
  return result;
...
static h2_pal_result_t
test_remote_channel_open_adapter(h2_pal_webrtc_peer_t *peer,
                                 h2_pal_webrtc_str_t label, uint16_t stream_id,
                                 int ordered, int reliable) {
  h2_pal_result_t result =
      (h2_peer_webrtc_on_remote_channel_open)(peer, label, stream_id, ordered,
                                              reliable);
  if (result == H2_PAL_OK) {
    for (size_t i = 0u;
         i < sizeof(s_peer_observers) / sizeof(s_peer_observers[0]); ++i) {
...
#define h2_pal_webrtc_peer_create(api, callbacks, out_peer)                    \
  test_peer_create_adapter((api), (callbacks), (out_peer))
#define h2_pal_webrtc_peer_poll(api, peer, timeout_ms)                         \
  test_peer_poll_adapter((api), (peer), (timeout_ms))
#define h2_pal_webrtc_peer_start_offer(api, peer)                              \
  test_peer_start_offer_adapter((api), (peer))
#define h2_pal_webrtc_peer_set_remote_sdp(api, peer, type, sdp)                \
  test_peer_set_remote_sdp_adapter((api), (peer), (type), (sdp))
#define h2_pal_webrtc_peer_create_data_channel(api, peer, config, out_channel) \
  test_peer_create_data_channel_adapter((api), (peer), (config), (out_channel))
#define h2_pal_webrtc_channel_close(api, channel)                              \
  test_channel_close_adapter((api), (channel))
#define h2_peer_receive_rtp_for_test(peer, packet, packet_len)                 \
  test_receive_rtp_adapter((peer), (packet), (packet_len))
#define h2_peer_webrtc_on_remote_channel_open(peer, label, stream_id, ordered, \
                                               reliable)                       \
  test_remote_channel_open_adapter((peer), (label), (stream_id), (ordered),    \
                                   (reliable))
...
static void test_reentrant_terminal_during_channel_open_preserves_result(void) {
  ...
  for (size_t i = 0u; i < 2u; ++i) {
    ...
    assert(h2_pal_webrtc_peer_create_data_channel(
               api, peer, &channel_config, &channel) == expected_results[i]);
    assert(channel == NULL);
...
static void test_reentrant_close_during_channel_open_is_deferred(void) {
  static const struct {
    ...
    size_t expected_channel_closed;
      ...
      {TEST_CHANNEL_OPEN_REENTRY_PEER_CLOSE, H2_PAL_OK, H2_PAL_ERR_CLOSED, 0u},
      {TEST_CHANNEL_OPEN_REENTRY_OWNER_DESTROY, H2_PAL_OK, H2_PAL_ERR_CLOSED,
       0u},
      {TEST_CHANNEL_OPEN_REENTRY_PEER_CLOSE, H2_PAL_ERR_IO, H2_PAL_ERR_IO, 0u},
  ...
  for (size_t i = 0u; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    assert(channel == NULL);
    assert(provider.channel_open == 1u);
    assert(callback_state.channel_closed == cases[i].expected_channel_closed);
    assert(callback_state.closed == 0u);
...
static void test_stream_reset_quarantine_and_ordering(void) {
  ...
  h2_pal_webrtc_peer_t *peer = NULL;
  assert(h2_pal_webrtc_peer_create(api, &callbacks, &peer) == H2_PAL_OK);
  h2_pal_webrtc_channel_config_t channel_config = test_channel_config("reset");
  ...
  assert(h2_pal_webrtc_peer_create_data_channel(api, peer, &independent_config,
                                                &independent) == H2_PAL_OK);
...
static void test_complete_lifecycle(void) {
  ...
  assert(h2_pal_webrtc_peer_start_offer(api, peer) == H2_PAL_OK);
  assert(callback_state.connecting == 1u && callback_state.local_sdp_len != 0u);
  ...
  h2_pal_webrtc_event_t timeout_event = {0};
  assert((h2_pal_webrtc_peer_poll)(api, peer, 1, &timeout_event) ==
         H2_PAL_ERR_TIMEOUT);
  ...
  h2_pal_webrtc_peer_close(api, peer);
  assert(provider.ice_close == 1u);
  assert(provider.dtls_close == 1u);
  assert(provider.srtp_close == 1u);
  assert(provider.sctp_close == 1u);
  assert(callback_state.closed == 0u && callback_state.channel_closed == 3u);
  h2_peer_destroy(&owner);
  assert(owner == NULL && mem.allocations == mem.frees);
}
...
static void test_connecting_callback_can_close_peer(void) {
  ...
  assert(h2_pal_webrtc_peer_create(api, &callbacks, &peer) == H2_PAL_OK);
  assert(h2_pal_webrtc_peer_start_offer(api, peer) == H2_PAL_OK);
  assert(callback_state.connecting == 1u && callback_state.closed == 0u);
...
static void test_media_track_poll_retains_opus_across_backpressure(void) {
  ...
  const h2_pal_webrtc_track_vtable_t track_vtable = {
...
static void test_media_track_lifecycle(void) {
  ...
  assert(h2_pal_webrtc_peer_unset_track(api, peer, &track) == H2_PAL_OK);
  assert(h2_pal_webrtc_peer_unset_track(api, peer, &track) ==
         H2_PAL_ERR_INVALID_STATE);
  h2_pal_webrtc_peer_close(api, peer);
  h2_peer_destroy(&owner);
  assert(mem.allocations == mem.frees);

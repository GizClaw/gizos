...
static void check_round_trip(const uint8_t *wire, size_t len) {
    ...
    assert(h2_darwin_corebluetooth_uuid_to_platform(
        &uuid, platform, sizeof(platform)) == H2_PAL_OK);
    assert(h2_darwin_corebluetooth_uuid_from_platform(
...
int main(void) {
    ...
    assert(h2_darwin_corebluetooth_adv_params_supported(&adv));
    adv.type = H2_PAL_BLE_ADV_TYPE_EXTENDED;
    assert(!h2_darwin_corebluetooth_adv_params_supported(&adv));
    assert(!h2_darwin_corebluetooth_adv_params_supported(NULL));
    h2_pal_ble_scan_params_t scan = {
        .type = H2_PAL_BLE_SCAN_TYPE_LEGACY,
    };
    assert(h2_darwin_corebluetooth_scan_params_supported(&scan));
    scan.type = H2_PAL_BLE_SCAN_TYPE_EXTENDED;
    assert(!h2_darwin_corebluetooth_scan_params_supported(&scan));
    assert(!h2_darwin_corebluetooth_scan_params_supported(NULL));
    return 0;
}

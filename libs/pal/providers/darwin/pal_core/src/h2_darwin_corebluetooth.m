...
static CBUUID *h2_corebluetooth_uuid(const h2_pal_ble_uuid_t *uuid) {
    ...
    uint8_t bytes[16];
    if (h2_darwin_corebluetooth_uuid_to_platform(
...
static size_t h2_corebluetooth_copy_uuid(
    ...
    if (h2_darwin_corebluetooth_uuid_from_platform(
...
- (h2_pal_result_t)start {
    ...
    while ([deadline timeIntervalSinceNow] > 0.0) {
        ...
        dispatch_sync(self.queue, ^{
            ready = self.centralReady && self.peripheralReady;
            if (self.centralStateKnown && self.peripheralStateKnown && !ready) {
                ready = NO;
            }
        });
        if (ready) {
        ...
        __block BOOL terminalState = NO;
        dispatch_sync(self.queue, ^{
            terminalState = self.centralStateKnown && self.peripheralStateKnown &&
                (!self.centralReady || !self.peripheralReady);
        });
        if (terminalState) {
...
- (h2_pal_result_t)createAdvertisingSet:(const h2_pal_ble_adv_params_t *)params
    ...
    if (!h2_darwin_corebluetooth_adv_params_supported(params)) {
...
- (h2_pal_result_t)startScan:(const h2_pal_ble_scan_params_t *)params
    ...
    if (!h2_darwin_corebluetooth_scan_params_supported(params)) {
        return H2_PAL_ERR_UNSUPPORTED;
    }
    dispatch_sync(self.queue, ^{
...
static h2_pal_ble_t s_h2_darwin_corebluetooth_api = {
    .user = NULL,

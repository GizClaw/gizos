...
static void dispatch_urc(void *user, const char *line) {
    h2_simcom_modem_t *modem = user;
    if (h2_pal_mutex_lock(modem->config.sync_api, modem->lock) != H2_PAL_OK) {
        return;
    }
...
h2_pal_result_t h2_simcom_modem_deinit(h2_simcom_modem_t *modem) {
    ...
    if (modem->opened != 0u) {
        h2_pal_result_t rc = h2_simcom_modem_close(&modem->platform, modem->config.command_timeout_ms);

...
static int dial_number_valid(
    ...
    for (size_t i = 0u; i < number_capacity; ++i) {
        ...
        if (!(h2_simcom_ascii_is_digit(ch) || ch == '+' || ch == '*' ||
              ch == '#' || ch == ',' || ch == 'p' || ch == 'P' ||
              ch == 'w' || ch == 'W')) {
            return 0;
        }
...
int h2_simcom_parse_clcc_line(const char *line, h2_pal_modem_call_status_t *out_status) {
    ...
    if (quote != NULL) {
        h2_simcom_copy_token(out_status->number, sizeof(out_status->number), quote);
    }
    return 1;
}
...
h2_pal_result_t h2_simcom_modem_call_dial(
    ...
    h2_pal_result_t rc = h2_simcom_at_exchange(modem, cmd, NULL, 0);
    if (rc == H2_PAL_OK) {
...
h2_pal_result_t h2_simcom_modem_call_answer(h2_pal_modem_t *platform, uint32_t timeout_ms) {
    (void)timeout_ms;
    h2_simcom_modem_t *modem = h2_simcom_from_platform(platform);
    h2_pal_result_t rc = modem != NULL ? h2_simcom_at_exchange(modem, "ATA", NULL, 0) : H2_PAL_ERR_INVALID_ARG;
    if (rc == H2_PAL_OK) {
        h2_pal_modem_call_status_t status;
        memset(&status, 0, sizeof(status));
        status.call_id = -1;
...
h2_pal_result_t h2_simcom_modem_call_hangup(h2_pal_modem_t *platform, uint32_t timeout_ms) {
    (void)timeout_ms;
    h2_simcom_modem_t *modem = h2_simcom_from_platform(platform);
    h2_pal_result_t rc = modem != NULL ? h2_simcom_at_exchange(modem, "ATH", NULL, 0) : H2_PAL_ERR_INVALID_ARG;
    if (rc == H2_PAL_OK) {
        h2_pal_modem_call_status_t status;
        memset(&status, 0, sizeof(status));
        status.call_id = -1;
...
h2_pal_result_t h2_simcom_modem_get_call_status(
    ...
    h2_pal_result_t rc = h2_simcom_at_exchange(modem, "AT+CLCC", &response, 0);
    if (rc != H2_PAL_OK) {
        return rc;
    }
    const char *line = h2_simcom_response_find(&response, "+CLCC:");
    if (line == NULL) {
        return H2_PAL_OK;
    }
    if (!h2_simcom_parse_clcc_line(line, out_status)) {

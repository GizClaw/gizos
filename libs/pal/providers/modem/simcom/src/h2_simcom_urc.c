...
static int parse_registration_urc(h2_simcom_modem_t *modem, const char *line, h2_pal_modem_status_t *out_status) {
    ...
    out_status->registration = h2_simcom_parse_registration_stat(stat);
    out_status->rat = H2_PAL_MODEM_RAT_LTE;
    return 1;
}

static int parse_packet_urc(h2_simcom_modem_t *modem, const char *line, h2_pal_modem_status_t *out_status) {
    int attached = 0;
    if (modem == NULL || out_status == NULL || !h2_simcom_parse_int_after(line, "+CGATT:", &attached)) {
        return 0;
    }
    memset(out_status, 0, sizeof(*out_status));
    out_status->capabilities = h2_simcom_modem_capabilities(modem);
...
static void post_call_status(
    ...
    if (status != NULL) {
        ...
        event.call.call_id = -1;
        event.call.state = H2_PAL_MODEM_CALL_STATE_ENDED;
    }
    h2_simcom_post_system_event(modem, type, &event, sizeof(event));
}
...
void h2_simcom_handle_urc_line(h2_simcom_modem_t *modem, const char *line) {
    ...
    if (strncmp(line, "+CLIP:", 6) == 0) {
        ...
        if (quote != NULL) {
            h2_simcom_copy_token(status.number, sizeof(status.number), quote);
        }
        post_call_status(modem, H2_PAL_SYSTEM_EVENT_TYPE_MODEM_CALL_INCOMING, &status);
        return;
    }
    ...
    if (strncmp(line, "+CLCC:", 6) == 0) {
        h2_pal_modem_call_status_t status;
        if (h2_simcom_parse_clcc_line(line, &status)) {
            post_call_status(
                modem,
                status.state == H2_PAL_MODEM_CALL_STATE_INCOMING
    ...
    if (strcmp(line, "NO CARRIER") == 0 ||
        ...
        post_call_status(modem, H2_PAL_SYSTEM_EVENT_TYPE_MODEM_CALL_ENDED, NULL);
        return;
    }

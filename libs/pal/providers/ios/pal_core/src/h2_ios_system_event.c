...
#define H2_IOS_SYSTEM_EVENT_MAX_SUBSCRIPTIONS 48u
...
typedef struct h2_ios_system_event_dispatch {
    h2_pal_system_event_subscription_t *subscription;
    h2_pal_system_event_handler_t handler;
    void *handler_user;
} h2_ios_system_event_dispatch_t;

static h2_pal_system_event_subscription_t
    s_subscriptions[H2_IOS_SYSTEM_EVENT_MAX_SUBSCRIPTIONS];
static pthread_mutex_t s_mutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t s_idle = PTHREAD_COND_INITIALIZER;
static int s_initialized;
static _Thread_local h2_pal_system_event_subscription_t
    *s_dispatching_subscription;

static int h2_ios_system_event_init(void *user) {
    (void)user;
...
static void h2_ios_system_event_deinit(void *user) {
    (void)user;
    pthread_mutex_lock(&s_mutex);
    s_initialized = 0;
    for (size_t i = 0u; i < H2_IOS_SYSTEM_EVENT_MAX_SUBSCRIPTIONS; ++i) {
...
static int h2_ios_system_event_post(
    void *user,
    ...
    h2_ios_system_event_dispatch_t dispatches[
        H2_IOS_SYSTEM_EVENT_MAX_SUBSCRIPTIONS];
    size_t count = 0u;
    pthread_mutex_lock(&s_mutex);
    if (s_initialized == 0) {
        pthread_mutex_unlock(&s_mutex);
    ...
    for (size_t i = 0u; i < H2_IOS_SYSTEM_EVENT_MAX_SUBSCRIPTIONS; ++i) {
        ...
        if (subscription->active != 0 && subscription->type == event->type &&
            ...
            dispatches[count++] = (h2_ios_system_event_dispatch_t){
    ...
    for (size_t i = 0u; i < count; ++i) {
        h2_pal_system_event_subscription_t *previous =
            s_dispatching_subscription;
        s_dispatching_subscription = dispatches[i].subscription;
        int handler_rc = dispatches[i].handler(
            dispatches[i].handler_user, event);
        s_dispatching_subscription = previous;
        pthread_mutex_lock(&s_mutex);
        --dispatches[i].subscription->in_flight;
        if (dispatches[i].subscription->active == 0 &&
            dispatches[i].subscription->in_flight == 0u) {
            memset(dispatches[i].subscription, 0,
                   sizeof(*dispatches[i].subscription));
        }
...
static int h2_ios_system_event_subscribe(
    void *user,
    h2_pal_system_event_type_t type,
    h2_pal_system_event_handler_t handler,
    void *handler_user,
...
static void h2_ios_system_event_unsubscribe(
    void *user,
    h2_pal_system_event_subscription_t *subscription) {
    (void)user;
    ...
    if (subscription == s_dispatching_subscription) {
        pthread_mutex_unlock(&s_mutex);
        return;
    }
...
static const h2_pal_system_event_vtable_t s_vtable = {
    .init = h2_ios_system_event_init,
    .deinit = h2_ios_system_event_deinit,
    .post = h2_ios_system_event_post,
    .subscribe = h2_ios_system_event_subscribe,
    .unsubscribe = h2_ios_system_event_unsubscribe,
};

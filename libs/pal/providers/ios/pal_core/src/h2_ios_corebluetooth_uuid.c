...
static int h2_ios_corebluetooth_uuid_len_valid(size_t len) {
...
int h2_ios_corebluetooth_uuid_to_platform(
    ...
    if (uuid == NULL || uuid->data == NULL || out == NULL ||
        !h2_ios_corebluetooth_uuid_len_valid(uuid->len) ||
        out_size < uuid->len) {
...
int h2_ios_corebluetooth_uuid_from_platform(
    ...
    if (bytes == NULL || out == NULL ||
        !h2_ios_corebluetooth_uuid_len_valid(len) || out_size < len) {
        return H2_PAL_ERR_INVALID_ARG;
    }

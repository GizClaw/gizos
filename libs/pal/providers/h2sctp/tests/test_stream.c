...
int main(void) {
    ...
    send_message(
        ...
        false,
        reopened_again,
        sizeof(reopened_again));
    (void)h2_sctp_test_pump(&pair, 32u);
    assert(pair.passive.message_count == 5u);
    assert(pair.passive.messages[4].stream_id == 2u);
    assert(memcmp(
               pair.passive.messages[4].data,
               reopened_again,
               sizeof(reopened_again)) == 0);
    assert_stale_forward_tsn_does_not_advance_stream(
        pair.passive.association, 2u, pair.now_ms);

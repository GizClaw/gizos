...
/* RFC 9260 Appendix B reflected CRC32C (Castagnoli, polynomial 0x1EDC6F41,
 * reflected 0x82F63B78), computed slice-by-4: four bytes per table step with a
 * 4 KiB constant table so that the per-packet cost stays linear in words, not
 * bits. The table is h2_sctp_crc32c_table[k][b] = CRC of byte b followed by k
 * zero bytes; it is generated once from the polynomial and checked against the
 * RFC 3720 known-answer vectors in tests/test_wire.c. */
static const uint32_t h2_sctp_crc32c_table[4][256] = {
    {
...
static uint32_t h2_sctp_crc32c_update(
    ...
    const uint8_t *data,
    size_t len) {
    /* Byte-at-a-time to a 4-byte boundary keeps every 32-bit load below
     * aligned, which matters on Xtensa and Cortex-M0 class cores. */
    while (len != 0u && ((uintptr_t)data & 3u) != 0u) {
        crc = h2_sctp_crc32c_table[0][(crc ^ *data) & 0xffu] ^ (crc >> 8u);
        ++data;
        --len;
    }
    while (len >= 4u) {
        /* Reflected CRC consumes the low byte first, so the word is read
         * little-endian regardless of host byte order. */
        const uint32_t word = (uint32_t)data[0] | ((uint32_t)data[1] << 8u) |
                              ((uint32_t)data[2] << 16u) |
                              ((uint32_t)data[3] << 24u);
        crc ^= word;
        crc = h2_sctp_crc32c_table[3][crc & 0xffu] ^
              h2_sctp_crc32c_table[2][(crc >> 8u) & 0xffu] ^
              h2_sctp_crc32c_table[1][(crc >> 16u) & 0xffu] ^
              h2_sctp_crc32c_table[0][crc >> 24u];
        data += 4u;
        len -= 4u;
    }
    while (len != 0u) {

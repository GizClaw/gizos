...
static const h2_bk_task_policy_t s_audio_policy = {
    .sdk_name = NULL,
    .core = 1u,
    .priority = 4u,
...
static const h2_bk_task_policy_t s_speed_policy = {
    .sdk_name = NULL,
    .core = 0u,
    .priority = 6u,
    .min_stack_size = 4096u,
...
static const h2_trie_route_t s_routes[] = {
    ...
    {"h2loader/uartcmd", H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_h2loader_policy},
    {"audio-system-music", H2_TRIE_ROUTE_EXACT, handle_policy, &s_audio_policy},
    {"audio-system-mic", H2_TRIE_ROUTE_EXACT, handle_policy, &s_audio_policy},
    {"bleikcp-speed/", H2_TRIE_ROUTE_PREFIX, handle_policy, &s_speed_policy},
};

enum {
  ROUTE_NODE_CAPACITY = 1u + H2_TRIE_LITERAL_NODE_COUNT("h2loader/appcmd") +
                        H2_TRIE_LITERAL_NODE_COUNT("h2loader/uartcmd") +
                        H2_TRIE_LITERAL_NODE_COUNT("audio-system-music") +
                        H2_TRIE_LITERAL_NODE_COUNT("audio-system-mic") +
                        H2_TRIE_LITERAL_NODE_COUNT("bleikcp-speed/"),
};
...
h2_pal_result_t h2_bk_layout_task_policy_install(void) {
  ...
  if (rc != H2_PAL_OK) {
    printf("H2_PAL_TASK_POLICY_FAIL unit=ap stage=routes reason=%d\n", rc);
    return rc;
  }
  ...
  if (rc == H2_PAL_OK) {
    printf("H2_PAL_TASK_POLICY_READY unit=ap layout=h2loader\n");
  } else {

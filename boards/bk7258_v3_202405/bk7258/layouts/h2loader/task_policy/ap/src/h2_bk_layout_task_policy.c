...
static h2_pal_result_t resolve_policy(void *user, const char *name,
  ...
  if (strcmp(name, "audio-system-music") == 0 ||
      strcmp(name, "audio-system-mic") == 0) {
    *out_policy = (h2_bk_task_policy_t){
        .sdk_name = NULL,
        .core = 1u,
        .priority = 4u,
  ...
  static const char speed_prefix[] = "bleikcp-speed/";
  if (strncmp(name, speed_prefix, sizeof(speed_prefix) - 1u) == 0 &&
      name[sizeof(speed_prefix) - 1u] != '\0') {
    *out_policy = (h2_bk_task_policy_t){
        .sdk_name = NULL,
        .core = 0u,
        .priority = 6u,

...
static const h2_trie_route_t s_routes[] = {
    ...
    {"bleikcp-speed/kcp", H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_7_policy},
    {"bleikcp-speed/server", H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_5_policy},
};
...
enum {
  ROUTE_NODE_CAPACITY = 1u + H2_TRIE_LITERAL_NODE_COUNT("h2loader/appcmd") +
                        ...
                        H2_TRIE_LITERAL_NODE_COUNT("bleikcp-speed/kcp") +
                        H2_TRIE_LITERAL_NODE_COUNT("bleikcp-speed/server"),
};
...
h2_pal_result_t h2_esp_layout_task_policy_install(void) {
  ...
  if (rc == H2_PAL_OK) {
    printf("H2_PAL_TASK_POLICY_READY unit=esp layout=amoled\n");
  } else {

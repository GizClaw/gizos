...
static esp_err_t read_registers(uint8_t reg, uint8_t *out, size_t len) {
    return i2c_master_transmit_receive(
        s_touch.device, &reg, sizeof(reg), out, len, H2_FT3168_TIMEOUT_MS);
}

static esp_err_t write_register(uint8_t reg, uint8_t value) {
    const uint8_t data[] = {reg, value};
    return i2c_master_transmit(
        s_touch.device, data, sizeof(data), H2_FT3168_TIMEOUT_MS);
}

static esp_err_t reset_touch(void) {
    esp_err_t err = h2_esp_amoled_board_io_update_outputs(
        H2_FT3168_RESET_MASK, 0u);
    if (err != ESP_OK) {
        return err;
    }
    vTaskDelay(pdMS_TO_TICKS(20u));
    err = h2_esp_amoled_board_io_update_outputs(
        H2_FT3168_RESET_MASK, H2_FT3168_RESET_MASK);
    if (err == ESP_OK) {
...
static h2_pal_result_t touch_open(void *user) {
    ...
    if (err == ESP_OK) {
        err = i2c_master_probe(
            bus, H2_FT3168_I2C_ADDRESS, H2_FT3168_TIMEOUT_MS);
    }
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "FT3168 probe at 0x38 failed: %s", esp_err_to_name(err));
        (void)h2_esp_amoled_board_io_update_outputs(
            H2_FT3168_RESET_MASK, 0u);
...
static h2_pal_result_t touch_poll_event(
    ...
    const int32_t x = clamp_coordinate(
        ((int32_t)(data[1] & 0x0fu) << 8) | data[2],
        H2_FT3168_WIDTH);
    const int32_t y = clamp_coordinate(
        ((int32_t)(data[3] & 0x0fu) << 8) | data[4],
        H2_FT3168_HEIGHT);

 ...
 *
 * The caller must return from app_main after this function succeeds. The entry
 * function must release Runtime before returning; the task wrapper then calls
 * h2_esp_board_runtime_deinit() before deleting itself.
 */

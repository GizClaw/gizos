...
#define H2_P4_BOOT_BUTTON_ID 101u
#define H2_P4_SOC_TEMPERATURE_ID 501u
#define H2_P4_BOOT_BUTTON_GPIO GPIO_NUM_0
...
static int known_non_button_id(h2_pal_periph_id_t id) {
    return id == H2_P4_SOC_TEMPERATURE_ID;
}

static int known_non_temperature_id(h2_pal_periph_id_t id) {
    return id == H2_P4_BOOT_BUTTON_ID;
}
...
static h2_pal_result_t read_single_button(
    ...
    if (!s_boot_button_configured) {
        const gpio_config_t config = {
            .pin_bit_mask = 1ULL << H2_P4_BOOT_BUTTON_GPIO,
    ...
    const int level = gpio_get_level(H2_P4_BOOT_BUTTON_GPIO);
...
static h2_pal_result_t read_temperature(
    ...
    (void)user;
    if (id != H2_P4_SOC_TEMPERATURE_ID) {

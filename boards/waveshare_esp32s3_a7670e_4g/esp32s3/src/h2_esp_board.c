...
h2_pal_modem_t *h2_esp_board_modem(void) {
    static h2_esp_simcom_modem_t *modem;
    if (modem == NULL) {
        const h2_esp_simcom_modem_config_t config = {
            ...
            .system_events = h2_esp_board_system_event_api(),
        };
        h2_pal_result_t rc = h2_esp_simcom_modem_create(&config, &modem);
        if (rc != H2_PAL_OK) {
            printf("H2_WAVESHARE_A7670E_MODEM_CREATE rc=%d\n", rc);

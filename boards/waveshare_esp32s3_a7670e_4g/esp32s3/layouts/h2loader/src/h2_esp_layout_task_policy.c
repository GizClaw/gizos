...
static const h2_trie_route_t s_routes[] = {
    ...
    {"net/modem_ppp_rx", H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_7_policy},
    {"modem/call_in", H2_TRIE_ROUTE_EXACT, handle_policy, &s_priority_6_policy},
    {"modem/gnss_fix", H2_TRIE_ROUTE_EXACT, handle_policy,
     &s_priority_5_policy},
};
...
enum {
  ROUTE_NODE_CAPACITY = 1u + H2_TRIE_LITERAL_NODE_COUNT("h2loader/appcmd") +
                        ...
                        H2_TRIE_LITERAL_NODE_COUNT("net/modem_ppp_rx") +
                        H2_TRIE_LITERAL_NODE_COUNT("modem/call_in") +
                        H2_TRIE_LITERAL_NODE_COUNT("modem/gnss_fix"),
};
...
h2_pal_result_t h2_esp_layout_task_policy_install(void) {
  ...
  if (rc != H2_PAL_OK) {
    printf("H2_PAL_TASK_POLICY_FAIL unit=esp stage=routes reason=%d\n", rc);
    return rc;
  }
  ...
  if (rc == H2_PAL_OK) {
    printf("H2_PAL_TASK_POLICY_READY unit=esp "
           "layout=waveshare_esp32s3_a7670e_4g\n");
  } else {

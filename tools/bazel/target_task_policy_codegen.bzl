...
def render_policy_test(label, unit, tasks, default_policy, allocator):
    ...
    lines = [
        ...
        "static h2_pal_mem_api_t s_stack_allocator;",
        "static h2_pal_result_t configure_stacks(const h2_esp_task_policy_config_t *config) {",
        "  assert(config->psram_stack_allocator == NULL);",
        "  h2_esp_task_policy_config_t owned = *config;",
        "  owned.psram_stack_allocator = &s_stack_allocator;",
        "  return h2_esp_platform_task_configure(&owned);",
        "}",
        "",
    ...
    if flavor.unit == "esp":
        lines.extend([
            ...
            "  assert(%s_with_configure(configure_stacks) == H2_PAL_OK);" % flavor.install,
            "  assert(s_config.psram_stack_allocator == &s_stack_allocator);",
        ])
    if flavor.allocator:
    ...
    lines.extend([
        ...
        "  assert(%s_with_configure(configure_stacks) == H2_PAL_ERR_INVALID_STATE);" % flavor.install,
    ] if flavor.unit == "esp" else []) + [
        "  return 0;",
        "}",
        "",

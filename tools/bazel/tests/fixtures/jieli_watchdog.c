...
int main(int argc,char **argv) {
  ...
  if (!strcmp(argv[1],"boot")) {
    ...
    assert(atomic_load(&feeds)==1000u);
    ...
    unsigned before=atomic_load(&feeds);
    runner((void *)1);
    assert(atomic_load(&feeds)-before<=1u);
  } else return 2;
  return 0;
}

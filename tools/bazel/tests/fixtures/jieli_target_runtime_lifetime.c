...
int os_tasks_num_query(void) { assert(h2_atomic_load(&live)); return 12; }
int h2_jieli_ac791n_devkit_audio_idle_probe(h2_jieli_ac791n_devkit_audio_idle_t *out) {
 assert(h2_atomic_load(&live)); memset(out, 0, sizeof(*out));
...
void h2_smoke_audio_system_get_stats(h2_smoke_audio_system_stats_t *out) {
 assert(h2_atomic_load(&live) && audio_active); *out = (h2_smoke_audio_system_stats_t){1u, 1u};
}
int h2_pal_time_get_monotonic_ms(const void *time, uint64_t *out) {
 (void)time; assert(h2_atomic_load(&live)); *out = (uint64_t)h2_atomic_load(&sleep_calls) * 10u; return 0;
}
#if WORKER_TARGET
#if !CYCLE_TARGET
...
static h2_atomic_int_t allow_finish;
...
int h2_runtime_init(const h2_runtime_config_t *c, h2_runtime_t **out) {
 ...
 assert(!h2_atomic_exchange(&live, 1)); ++inits; *out = &instance; return 0;
}
void h2_runtime_deinit(h2_runtime_t *r) {
 assert(r == &instance && !retained && !audio_active); assert(h2_atomic_exchange(&live, 0)); ++deinits;
}
int h2_runtime_input_start(h2_runtime_t *r, const void *c) { (void)c; assert(r == &instance && h2_atomic_load(&live)); return fault == 3 ? -13 : 0; }
int h2_pal_time_sleep_ms(const void *time, unsigned ms) { (void)time; assert(ms == 10u || (CYCLE_TARGET && (ms == 3000u || ms == 2000u))); h2_atomic_fetch_add(&sleep_calls, 1); assert(h2_atomic_load(&live)); os_time_dly(1); return 0; }
int h2_smoke_audio_system_run(h2_runtime_t *r, const h2_smoke_audio_system_config_t *c) { (void)c; assert(r == &instance && h2_atomic_load(&live)); assert(!audio_active); audio_active = 1; stop_attempts = 0; ++audio_runs; return fault == 5 || (!CYCLE_TARGET && (fault == 4 || fault == 7)) ? -15 : 0; }
int h2_smoke_audio_system_stop(void) {
 assert(h2_atomic_load(&live) && audio_active);
 ++audio_stop_calls;
...
void cpu_assert_debug(void) { assert(h2_atomic_load(&live)); }
int h2_crash_before_confirm_run(h2_runtime_t *r, const h2_crash_before_confirm_config_t *c) { (void)c; assert(r == &instance && h2_atomic_load(&live)); return -15; }
int h2_pal_display_open(const void *d) { (void)d; assert(h2_atomic_load(&live)); return fault == 3 ? -13 : 0; }
int h2_pal_fs_stat(const void *fs, const char *path, h2_pal_fs_stat_t *st) { (void)fs; (void)path; (void)st; assert(h2_atomic_load(&live)); return H2_PAL_ERR_NOT_FOUND; }
...
static int smoke_run(h2_runtime_t *r, const smoke_config_t *c) {
 assert(r == &instance && h2_atomic_load(&live));
 if (fault == 6) { while (!c->should_stop(c->stop_user)) os_time_dly(1); return -8; }
 c->on_started(c->started_user, fault == 5 ? -15 : 0);
 if (fault != 5) while (!h2_atomic_load(&allow_finish)) { assert(h2_atomic_load(&live)); os_time_dly(1); }
 return fault == 5 ? -15 : 0;
}
...
static void h2_fixture_atomic_cleanup(void) {
    h2_atomic_destroy(&live);
    h2_atomic_destroy(&finishes);
    h2_atomic_destroy(&sleep_calls);
#if WORKER_TARGET
    h2_atomic_destroy(&allow_finish);
#endif
}
int main(void) {
    assert(atexit(h2_fixture_atomic_cleanup) == 0);
    assert(h2_atomic_init(&live, 0) == H2_ATOMIC_OK);
    assert(h2_atomic_init(&finishes, 0) == H2_ATOMIC_OK);
    assert(h2_atomic_init(&sleep_calls, 0) == H2_ATOMIC_OK);
#if WORKER_TARGET
    assert(h2_atomic_init(&allow_finish, 0) == H2_ATOMIC_OK);
#endif

 const int faults[] = {7, 5, 1, 2, 3, 4, 6, 0};
 for (size_t i = 0; i < sizeof(faults) / sizeof(faults[0]); ++i) {
  fault = faults[i];
  h2_atomic_store(&sleep_calls, 0);
  inits = deinits = retained = audio_active = audio_stop_calls = stop_attempts = audio_runs = 0; h2_atomic_store(&live, 0); h2_atomic_store(&finishes, 0);
#if WORKER_TARGET
  worker_started = 0; h2_atomic_store(&allow_finish, 0);
#endif
  ...
  else { assert(result == 0 && h2_atomic_load(&live)); }
  h2_atomic_store(&allow_finish, 1);
  if (worker_started) assert(pthread_join(worker, NULL) == 0);
#else
...
#if CYCLE_TARGET
  if (fault == 7) {
   assert(inits == 1 && deinits == 0 && h2_atomic_load(&live) && audio_active);
   assert(audio_runs == 1 && audio_stop_calls == 100);
  } else {
   assert(inits == deinits && !h2_atomic_load(&live) && !audio_active);
...
#elif AUDIO_TARGET
  if (fault == 1 || fault == 2) {
   assert(!audio_stop_calls && !h2_atomic_load(&sleep_calls) && !deinits && !inits);
  }
  if (result == H2_AUDIO_OK) {
   /* Successful entry hands the still-running scene to the boot lifetime. */
   assert(inits == 1 && deinits == 0 && h2_atomic_load(&live) && audio_active && !audio_stop_calls);
  } else if (fault == 7) {
   assert(result == -15 && audio_stop_calls == 100 && h2_atomic_load(&sleep_calls) == 99);
   assert(inits == 1 && deinits == 0 && h2_atomic_load(&live) && audio_active);
  } else {
   assert(inits == deinits && !h2_atomic_load(&live) && !audio_active);
...
#else
  assert(inits == deinits && !h2_atomic_load(&live) && !retained);
#endif
 }
 return 0;
}

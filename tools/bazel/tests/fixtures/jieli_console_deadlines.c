...
#define BIT(n) (1u << (n))
static atomic_int held, block_lock, worker_entered, worker_release;
...
int os_mutex_accept(OS_MUTEX *mutex) {
    if (block_lock || pthread_mutex_trylock(mutex) != 0) return 1;
    held = 1;
    return 0;
}
...
int os_mutex_post(OS_MUTEX *mutex) { assert(held); held = 0; return pthread_mutex_unlock(mutex); }
...
int dev_write(void *device, void *buffer, u32 size) {
    assert(device == &handle && held && size <= 512);
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "threaded_read") == 0) {
        ...
        assert(now - before <= 7 && writes == 1 && !held);
        ...
        assert(writes == 2 && dma_buffer == NULL && !held);
    }
    return 0;
}

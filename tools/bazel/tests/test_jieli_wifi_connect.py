...
enum { SCAN_IDLE, SCAN_ABANDONED = 3 };
...
int main(void) {
    ...
    stop_order=0; sdk_on=1; off_result=-1; wifi_state.on=1; scan_phase=SCAN_ABANDONED;
    assert(wifi_stop()==H2_PAL_ERR_IO && stop_order==2 && wifi_state.on);
    assert(scan_phase==SCAN_ABANDONED); /* a failed radio stop keeps the scan owned */
    stop_order=0; off_result=0;
    assert(wifi_stop()==H2_PAL_OK && stop_order==3 && !wifi_state.on);
    assert(scan_phase==SCAN_IDLE);
    return 0;
}
'''
